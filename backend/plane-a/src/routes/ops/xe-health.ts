...
const logger = createLogger('plane-a.ops.xe-health')
...
export const xeHealthRoutes = async (app: FastifyInstance) => {
  app.get('/api/ops/xe/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('xe')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('xe_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('xe_health_success', {
      ...
      return {
        ...
        message: 'Failed to fetch XE health data',
      }
    }
  })
}
