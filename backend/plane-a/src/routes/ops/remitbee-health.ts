...
const logger = createLogger('plane-a.ops.remitbee-health')
...
export const remitbeeHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/remitbee/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('remitbee')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('remitbee_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('remitbee_health_success', {
      ...
      logger.error('remitbee_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Remitbee health data',
      }
    }
  })
}
