...
const logger = createLogger('plane-a.ops.bossmoney-health')
...
export const bossmoneyHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/bossmoney/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('bossmoney')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('bossmoney_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('bossmoney_health_success', {
      ...
      logger.error('bossmoney_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Boss Money health data',
      }
    }
  })
}
