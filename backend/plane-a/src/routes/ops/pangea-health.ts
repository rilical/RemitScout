...
const logger = createLogger('plane-a.ops.pangea-health')
...
export const pangeaHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/pangea/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('pangea')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('pangea_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('pangea_health_success', {
      ...
      logger.error('pangea_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Pangea health data',
      }
    }
  })
}
