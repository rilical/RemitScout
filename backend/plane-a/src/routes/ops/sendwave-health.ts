...
const logger = createLogger('plane-a.ops.sendwave-health')
...
export const sendwaveHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/sendwave/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('sendwave')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('sendwave_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('sendwave_health_success', {
      ...
      logger.error('sendwave_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Sendwave health data',
      }
    }
  })
}
