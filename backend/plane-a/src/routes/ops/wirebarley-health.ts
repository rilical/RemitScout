...
const logger = createLogger('plane-a.ops.wirebarley-health')
...
export const wirebarleyHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/wirebarley/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('wirebarley')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('wirebarley_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('wirebarley_health_success', {
      ...
      logger.error('wirebarley_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch WireBarley health data',
      }
    }
  })
}
