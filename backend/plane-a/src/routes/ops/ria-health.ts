...
const logger = createLogger('plane-a.ops.ria-health')
...
export const riaHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/ria/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('ria')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('ria_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('ria_health_success', {
      ...
      logger.error('ria_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch RIA health data',
      }
    }
  })
}
