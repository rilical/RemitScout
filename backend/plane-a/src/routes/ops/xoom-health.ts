...
const logger = createLogger('plane-a.ops.xoom-health')
...
export const xoomHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/xoom/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('xoom')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('xoom_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('xoom_health_success', {
      ...
      logger.error('xoom_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Xoom health data',
      }
    }
  })
}
