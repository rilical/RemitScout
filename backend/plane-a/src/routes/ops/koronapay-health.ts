...
const logger = createLogger('plane-a.ops.koronapay-health')
...
export const koronapayHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/koronapay/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('koronapay')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('koronapay_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('koronapay_health_success', {
      ...
      logger.error('koronapay_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch KoronaPay health data',
      }
    }
  })
}
