...
const logger = createLogger('plane-a.ops.mukuru-health')
...
export const mukuruHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/mukuru/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('mukuru')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('mukuru_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('mukuru_health_success', {
      ...
      logger.error('mukuru_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Mukuru health data',
      }
    }
  })
}
