...
export const wiseHealthRoutes = async (app: FastifyInstance) => {
  app.get('/api/ops/wise/health', { preHandler: requireAdmin() }, async () => {
    ...
    const corridors = WISE_HEALTH_CORRIDORS.map((corridorId) => {
