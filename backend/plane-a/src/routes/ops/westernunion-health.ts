...
export const westernUnionHealthRoutes = async (app: FastifyInstance) => {
  app.get('/api/ops/westernunion/health', { preHandler: requireAdmin() }, async () => {
    ...
    const corridors = WESTERNUNION_HEALTH_CORRIDORS.map((corridorId) => {
