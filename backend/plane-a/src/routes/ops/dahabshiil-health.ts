...
const logger = createLogger('plane-a.ops.dahabshiil-health')
...
export const dahabshiilHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/dahabshiil/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('dahabshiil')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('dahabshiil_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('dahabshiil_health_success', {
      ...
      logger.error('dahabshiil_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Dahabshiil health data',
      }
    }
  })
}
