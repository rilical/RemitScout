...
export const remitlyHealthRoutes = async (app: FastifyInstance) => {
  app.get('/api/ops/remitly/health', { preHandler: requireAdmin() }, async () => {
    ...
    const corridors = REMITLY_HEALTH_CORRIDORS.map((corridorId) => {
