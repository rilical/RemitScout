...
export const worldRemitHealthRoutes = async (app: FastifyInstance) => {
  app.get('/api/ops/worldremit/health', { preHandler: requireAdmin() }, async () => {
    ...
    const corridors = WORLDREMIT_HEALTH_CORRIDORS.map((corridorId) => {
