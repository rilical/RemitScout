...
const logger = createLogger('plane-a.ops.placid-health')
...
export const placidHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/placid/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('placid')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('placid_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('placid_health_success', {
      ...
      logger.error('placid_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Placid health data',
      }
    }
  })
}
