...
const logger = createLogger('plane-a.ops.instarem-health')
...
export const instaremHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/instarem/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('instarem')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('instarem_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('instarem_health_success', {
      ...
      logger.error('instarem_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Instarem health data',
      }
    }
  })
}
