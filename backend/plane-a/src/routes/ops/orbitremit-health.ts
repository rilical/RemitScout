...
const logger = createLogger('plane-a.ops.orbitremit-health')
...
export const orbitremitHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/orbitremit/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('orbitremit')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('orbitremit_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('orbitremit_health_success', {
        corridor_count: corridors.length,
        stale_count: staleCorridors.length,
      })

      const metadata = getProviderMetadata('orbitremit')
      ...
      logger.error('orbitremit_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch OrbitRemit health data',
      }
    }
  })
}
