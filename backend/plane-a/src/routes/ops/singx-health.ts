...
const logger = createLogger('plane-a.ops.singx-health')
...
export const singxHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/singx/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('singx')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('singx_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('singx_health_success', {
        corridor_count: corridors.length,
        stale_count: staleCorridors.length,
      })

      const metadata = getProviderMetadata('singx')
      ...
      logger.error('singx_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch SingX health data',
      }
    }
  })
}
