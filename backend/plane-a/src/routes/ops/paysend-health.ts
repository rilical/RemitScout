...
const logger = createLogger('plane-a.ops.paysend-health')
...
export const paysendHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/paysend/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('paysend')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('paysend_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('paysend_health_success', {
      ...
      logger.error('paysend_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Paysend health data',
      }
    }
  })
}
