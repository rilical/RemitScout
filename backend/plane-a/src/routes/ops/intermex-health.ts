...
const logger = createLogger('plane-a.ops.intermex-health')
...
export const intermexHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/intermex/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('intermex')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('intermex_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('intermex_health_success', {
      ...
      logger.error('intermex_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Intermex health data',
      }
    }
  })
}
