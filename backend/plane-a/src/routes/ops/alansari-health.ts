...
const logger = createLogger('plane-a.ops.alansari-health')
...
export const alansariHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/alansari/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('alansari')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('alansari_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('alansari_health_success', {
      ...
      logger.error('alansari_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch Al Ansari health data',
      }
    }
  })
}
