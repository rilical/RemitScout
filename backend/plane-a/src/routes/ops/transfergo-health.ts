...
const logger = createLogger('plane-a.ops.transfergo-health')
...
export const transferGoHealthRoutes = async (app: FastifyInstance) => {
  app.get('/ops/transfergo/health', { preHandler: requireAdmin() }, async (request, reply) => {
    const healthCorridors = getHealthCorridors('transfergo')
    if (!healthCorridors || healthCorridors.length === 0) {
      logger.warn('transfergo_health_no_corridors')
      reply.code(500)
      return {
    ...
    try {
      ...
      logger.debug('transfergo_health_success', {
      ...
      logger.error('transfergo_health_failed', {
        error: errorMessage,
        stack: errorStack,
      })
      reply.code(500)
      return {
        ...
        message: 'Failed to fetch TransferGo health data',
      }
    }
  })
}
