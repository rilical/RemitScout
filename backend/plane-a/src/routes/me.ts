...
const buildBillingInfo = async (
  ...
  try {
    ...
    const currentPeriodEnd = toIsoFromSeconds(
      subscriptionWithPeriodEnd.current_period_end ?? null,
    );
  ...
  return Array.from(
    new Set(
      scopes
        ...
        .filter(
          (scope) => scope && scope !== "tier:1" && !isRetailApiKeyScope(scope),
        ),
    ),
  );
};

const verifySupabasePassword = async (
  email: string,
  password: string,
): Promise<boolean> => {
  ...
  throw new AuthenticationError(
    payload || `Supabase auth failed with status ${response.status}`,
  );
};
...
export const meRoutes = async (app: FastifyInstance) => {
    ...
    try {
      ...
      return {
        ...
        plan_effective: {
          ...
          is_active: effective.internalEnterpriseOverride
            ? true
            : effective.isPlanActive,
      ...
      try {
        ...
        if (!effective.entitlements.api_access) {
          return denyCapability(reply, effective, {
            ...
            insufficientMessage:
              "Enterprise API access is required for API keys.",
            inactiveMessage:
              "Your paid plan is inactive. Reactivate billing to manage API keys.",
          });
        }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const unsupportedScopes = findUnsupportedApiKeyScopes(
          parsed.data.scopes,
        );
        ...
        if (hasTierOneScope(parsed.data.scopes)) {
          throw new ValidationError("Invalid request", {
            details: {
              error: "tier_disabled",
              message: "Tier 1 API access is disabled.",
            },
          });
        }
        ...
        const maxKeys =
          effective.entitlements.api_key_max > 0
    ...
    async (request, reply) => {
      ...
      try {
        const limit = Math.min(
          Number((request.query as { limit?: unknown } | undefined)?.limit) ||
            100,
          200,
        );
        const publishedEmbeds =
          await app.container.repositories.publishedEmbed.listByOwnerUserId(
            user.user_id,
            limit,
          );
        return {
          success: true,
          embeds: publishedEmbeds.map((row) =>
            buildPublishedEmbedListItem(row),
          ),
        };
      } catch (error: unknown) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const revoked = await app.container.repositories.publishedEmbed.revoke(
          parsedId.data,
          user.user_id,
        );
        if (!revoked) {
          ...
          await logAuditEvent(planeAPool, {
            ...
            afterSnapshot: {
              revoked_at: revoked.revoked_at
                ? revoked.revoked_at.toISOString()
                : null,
      ...
      try {
        ...
        if (!effective.entitlements.bulk_export) {
          return denyCapability(reply, effective, {
            ...
            inactiveMessage:
              "Your paid plan is inactive. Reactivate billing to manage export jobs.",
          });
        }
        ...
        const jobs = await exportJobRepository.listByUserId(
          user.user_id,
          limit,
        );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (
          parsed.data.jobType === "indices" &&
          !effective.entitlements.indices_exports_enabled
        ) {
          return denyCapability(reply, effective, {
            ...
            insufficientMessage:
              "TEER, RCI, and RVI exports require an Enterprise plan.",
            inactiveMessage:
              "Your paid plan is inactive. Reactivate billing to create TEER, RCI, and RVI exports.",
          });
        }
        if (
          parsed.data.jobType === "indices" &&
          (!parsed.data.corridorIds || parsed.data.corridorIds.length === 0)
        ) {
          throw new ValidationError("Invalid request", {
            details: {
        ...
        if (count >= 10) {
          ...
          return {
            error: "rate_limited",
            message: "Maximum 10 export jobs per 24 hours.",
          };
        }
      ...
      try {
        ...
        if (!effective.entitlements.bulk_export) {
          return denyCapability(reply, effective, {
            ...
            insufficientMessage: "Enterprise bulk export access is required.",
            inactiveMessage:
              "Your paid plan is inactive. Reactivate billing to download export jobs.",
          });
        }
  ...
  app.patch("/me", { preHandler: requireAuth() }, async (request, reply) => {
    ...
    try {
      await upsertUserAccount(planeAPool, user);
      const beforeProfile = await userAccountRepository.getProfile(
        user.user_id,
      );
      ...
      if (error instanceof z.ZodError) {
        throw new ValidationError("Invalid request", {
          details: { error: "invalid_request", details: error.flatten() },
        });
      }
