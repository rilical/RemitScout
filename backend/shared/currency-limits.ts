 ...
 * These are approximate rates used for validation purposes.
 */
...
export const getMinAmount = (currencyCode: string): number => {
  const currency = currencyCode.toUpperCase()
  const rate = FIXED_EXCHANGE_RATES[currency] || 1.0
  const minAmount = MIN_AMOUNT_USD * rate

  if (minAmount >= 1000) {
    return Math.ceil(minAmount / 100) * 100
  }
  if (minAmount >= 100) {
    return Math.ceil(minAmount / 10) * 10
  }
  if (minAmount >= 10) {
...
export const getMaxAmount = (currencyCode: string): number => {
  const currency = currencyCode.toUpperCase()
  const rate = FIXED_EXCHANGE_RATES[currency] || 1.0
  const maxAmount = MAX_AMOUNT_USD * rate

  if (maxAmount >= 1000000) {
    return Math.floor(maxAmount / 10000) * 10000
  }
  if (maxAmount >= 100000) {
    return Math.floor(maxAmount / 1000) * 1000
  }
  if (maxAmount >= 10000) {
    return Math.floor(maxAmount / 100) * 100
  }
  if (maxAmount >= 1000) {
...
export const sanitizeAmount = (
  ...
  let clamped = Math.max(0, numAmount)
  if (minAmount !== null) {
