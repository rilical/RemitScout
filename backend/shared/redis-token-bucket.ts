...
const logger = createLogger('shared.redis-token-bucket')
...
export class RedisTokenBucket {
   ...
   * @param maxWaitMs - Optional cap on total wait time before failing
   * @throws Error if acquisition fails after max retries or invalid requested value
   */
  async acquireToken(requested = 1, maxWaitMs?: number): Promise<void> {
    ...
    while (retries < MAX_RETRIES) {
      ...
      if (Number.isFinite(maxWaitMs) && maxWaitMs !== undefined) {
        const elapsedMs = Date.now() - startedAt
        if (elapsedMs + waitMs > maxWaitMs) {
          throw new Error(`token_bucket_timeout:${this.key}`)
        }
      }
