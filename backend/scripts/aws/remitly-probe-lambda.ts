/**
 * Remitly Provider Probe Lambda Handler
 * 
 * AWS Lambda wrapper for Remitly provider health probe.
 * Resolves AWS parameters and calls the generic probe.
 */
...
const logger = createLogger('script.remitly-probe-lambda')
const PROVIDER_ID = 'remitly'
...
export const handler = async (): Promise<{ success: boolean; result?: unknown; error?: string }> => {
  ...
  try {
      ...
      await resolveAwsEnv([
        {
          ...
          required: false, // Redis is optional for probes
        },
      ])
    } catch (error: unknown) {
      const { message, stack } = formatError(error)
    ...
    const result = await runGenericProbe({
      ...
      timeoutMs: Math.min(Number(process.env.PROBE_TIMEOUT_MS) || 300000, lambdaTimeoutMs - 10000), // Leave 10s buffer
