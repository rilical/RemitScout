...
const logger = createLogger('script.gold-pulse-cache-lambda')
...
export const handler = async (): Promise<{ status: string }> => {
  try {
    ...
    const { runGoldPulseCacheJob } = await import('../gold-pulse-cache-job')
    await runGoldPulseCacheJob({ enableHealthServer: false })
    return { status: 'ok' }
  } catch (error) {
