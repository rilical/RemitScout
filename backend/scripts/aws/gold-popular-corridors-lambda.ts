...
const logger = createLogger('script.gold-popular-corridors-lambda')
...
export const handler = async (): Promise<{ status: string }> => {
  try {
    ...
    const { runGoldPopularCorridorsJob } = await import('../gold-popular-corridors-job')
    await runGoldPopularCorridorsJob({ enableHealthServer: false })
    return { status: 'ok' }
  } catch (error) {
