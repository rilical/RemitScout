...
const logger = createLogger('script.gold-indices-job-lambda')
...
export const handler = async (): Promise<{ status: string }> => {
  try {
    ...
    const { runGoldIndicesJob } = await import('../gold-indices-job')
    await runGoldIndicesJob({ enableHealthServer: false })
    return { status: 'ok' }
  } catch (error) {
