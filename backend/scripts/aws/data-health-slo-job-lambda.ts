/**
 * Lambda entrypoint for Data Health SLO Job.
 */
...
const logger = createLogger('script.data-health-slo-job-lambda')
...
export const handler = async (): Promise<{ statusCode: number; body: string }> => {
  try {
    ...
    try {
      const { runDataHealthSloJob } = await import('../data-health-slo-job')
      await runDataHealthSloJob()
      return {
        statusCode: 200,
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'data_health_slo_job_failed', message }),
      }
    }
  } finally {
