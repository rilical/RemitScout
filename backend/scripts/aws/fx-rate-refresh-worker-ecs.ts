/**
 * FX Rate Refresh Worker ECS Entry Point
 *
 * ECS task entry point for FX rate refresh worker.
...
export const handler = async (): Promise<number> => {
  try {
    ...
    logger.error('database_url_resolution_failed', {
      job_name: 'fx-rate-refresh-worker',
      env_vars_attempted: [
  ...
  const { isShutdownRequested } = createShutdownHandler({
    ...
    logger,
    onShutdown: async () => {
      logger.info('fx_rate_refresh_worker_shutdown', { reason: 'shutdown_requested' })
    },
  })

  if (isShutdownRequested()) {
    logger.info('fx_rate_refresh_worker_skipped', { reason: 'shutdown_requested' })
    return 0
  }
  ...
  try {
    ...
    const { message, stack } = formatError(error)
    logger.error('fx_rate_refresh_worker_failed', {
      error: message,
      stack,
    })
    throw error
...
if (require.main === module && !process.env.AWS_LAMBDA_FUNCTION_NAME) {
  handler()
    ...
    .catch((error: unknown) => {
      const { message, stack } = formatError(error)
      logger.error('fx_rate_refresh_worker_fatal', {
        error: message,
        stack,
      })
      process.exit(1)
