...
const logger = createLogger('script.quote-refresh-queue-cleanup-lambda')
...
export const handler = async (): Promise<{ status: string }> => {
  try {
    ...
    const { runQuoteRefreshQueueCleanup } = await import('../quote-refresh-queue-cleanup')
    await runQuoteRefreshQueueCleanup()
    return { status: 'ok' }
  } catch (error) {
