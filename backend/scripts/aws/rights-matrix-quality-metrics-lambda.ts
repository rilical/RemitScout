...
const logger = createLogger('script.rights-matrix-quality-metrics-lambda')
...
export const handler = async (): Promise<{ status: string }> => {
  try {
    ...
    const { runRightsMatrixQualityMetrics } = await import('../rights-matrix-quality-metrics-job')
    await runRightsMatrixQualityMetrics()
    return { status: 'ok' }
  } catch (error) {
