...
const logger = createLogger('script.gold-publisher-lambda')
...
export const handler = async (): Promise<{ status: string }> => {
  try {
    ...
    const { runGoldPublisherJob } = await import('../gold-publisher-job')
    await runGoldPublisherJob({ enableHealthServer: false })
    return { status: 'ok' }
  } catch (error) {
