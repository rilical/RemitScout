...
import { getMetrics, metricsContentType } from './gold-pulse-cache-job-metrics'
...
export const startHealthServer = async (
  ...
  return startSharedHealthServer({
    ...options,
    loggerName: 'script.gold-pulse-cache-health',
    getMetrics,
    metricsContentType,
...
if (require.main === module) {
  const logger = createLogger('script.gold-pulse-cache-health')
