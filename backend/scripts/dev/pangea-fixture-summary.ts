/**
 * Summarize Pangea fixtures by corridor and method pairs.
 *
...
const amount = Number(process.env.PANGEA_FIXTURE_AMOUNT || 100)
...
const main = async () => {
  ...
  for (const fixture of fixtures) {
    ...
    const payload = JSON.parse(raw) as PangeaPayload
    const pairs = extractPangeaMethodPairs(payload)
    ...
    const parsed = parsePangeaPayload(payload, buildRequest(fixture.corridorId))
