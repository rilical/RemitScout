/**
 * Dev-only utility to inspect Western Union payload method codes.
 */
...
const requestTimeoutMs = toNumber(process.env.WU_OBSERVE_TIMEOUT_MS, 25000)

const withTimeout = async <T>(promise: Promise<T>, timeoutMs: number, label: string): Promise<T> => {
  let timeoutId: ReturnType<typeof setTimeout> | null = null
  const timeoutPromise = new Promise<never>((_, reject) => {
    timeoutId = setTimeout(() => {
      reject(new Error(`WU observe timeout for ${label} after ${timeoutMs}ms`))
    }, timeoutMs)
  })
...
const main = async () => {
  ...
  for (const corridorId of corridors) {
    ...
    const result = await withTimeout(
      fetchWesternUnionQuote(request, { forceAllPayins: true, jitterMs: 250 }),
      requestTimeoutMs,
      corridorId,
    )
