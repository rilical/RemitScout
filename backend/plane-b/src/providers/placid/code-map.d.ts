export declare const payinMethodMap: Record<string, string>;
export declare const payoutMethodMap: Record<string, string>;
export declare const mapPayinMethod: (value?: string | null) => string;
export declare const mapPayoutMethod: (value?: string | null) => string;
export declare const mapPlacidPaymentType: (value?: string | null) => string;
export declare const mapPayinToPlacidCode: (value?: string | null) => string;
export declare const normalizeMethodToken: (value: string) => string;
