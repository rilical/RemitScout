...
import { fetchWellsFargoQuote } from './fetch'
import { parseWellsFargoPayload } from './parse'
...
const logger = createLogger('plane-b.wellsfargo.collector')
...
export const runWellsFargoCollector = async (options: WellsFargoCollectorOptions = {}) => {
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = WELLSFARGO_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.wellsfargo.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.wellsfargo.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.wellsfargo.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.wellsfargo.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.wellsfargo.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.wellsfargo.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.wellsfargo.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.wellsfargo.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.wellsfargo.freshnessSloEnabled
  const blockCooldownMs = config.planeB.wellsfargo.blockCooldownMs
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchWellsFargoQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchWellsFargoQuote(request, { jitterMs, proxyTier })
