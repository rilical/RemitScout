...
const logger = (0, logger_1.createLogger)('plane-b.xe.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractXeMethodPairs)(payload);
 ...
 */
const runXeCollector = async (options = {}) => {
    const providerId = 'xe';
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.XE_SUPPORTED_CORRIDORS;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'XE Money Transfer');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchXeQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseXePayload)(fetchResult.payload, request);
...
exports.runXeCollector = runXeCollector;
