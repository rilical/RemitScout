...
import { XE_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchXeQuote } from './fetch'
import { extractXeMethodPairs, parseXePayload } from './parse'
...
const logger = createLogger('plane-b.xe.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractXeMethodPairs(payload)
  ...
  await query(
    `INSERT INTO silver.provider_corridor_capability
     ...
     ON CONFLICT (provider_id, corridor_id) DO UPDATE SET
       ...
       updated_at = NOW()`,
    ['xe', corridorId, payinValue, payoutValue, true, 'observed'],
    pool,
  )
}
...
export const runXeCollector = async (options: XeCollectorOptions = {}) => {
  ...
  const corridors = resolvedCorridors.length ? resolvedCorridors : XE_SUPPORTED_CORRIDORS
  ...
  await ensureProvider(pool, providerId, 'XE Money Transfer')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchXeQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchXeQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parseXePayload(fetchResult.payload as Record<string, unknown>, request)
