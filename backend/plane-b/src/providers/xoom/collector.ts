...
import { XOOM_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchXoomQuote } from './fetch'
import { extractXoomMethodPairs, parseXoomPayload } from './parse'

/**
 * Xoom Collector
 *
 * This collector orchestrates the collection of money transfer quotes from Xoom's API.
...
const logger = createLogger('plane-b.xoom.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractXoomMethodPairs(payload)
...
/**
 * Main collector function that orchestrates quote collection from Xoom.
 *
 ...
 */
export const runXoomCollector = async (options: XoomCollectorOptions = {}) => {
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = XOOM_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.xoom.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.xoom.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.xoom.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.xoom.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.xoom.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.xoom.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.xoom.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.xoom.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.xoom.freshnessSloEnabled
  const blockCooldownMs = config.planeB.xoom.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'Xoom')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchXoomQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchXoomQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parseXoomPayload(fetchResult.payload as Record<string, unknown>, request)
