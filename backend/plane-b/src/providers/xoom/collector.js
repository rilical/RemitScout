...
exports.runXoomCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.xoom.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractXoomMethodPairs)(payload);
 ...
 */
const runXoomCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.XOOM_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.xoom.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.xoom.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.xoom.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.xoom.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.xoom.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.xoom.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.xoom.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.xoom.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.xoom.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.xoom.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Xoom');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchXoomQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseXoomPayload)(fetchResult.payload, request);
...
exports.runXoomCollector = runXoomCollector;
