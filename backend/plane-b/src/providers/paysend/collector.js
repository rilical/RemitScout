...
exports.runPaysendCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.paysend.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractPaysendMethodPairs)(payload, corridorId);
 ...
 */
const runPaysendCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.PAYSEND_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.paysend.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.paysend.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.paysend.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.paysend.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.paysend.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.paysend.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.paysend.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.paysend.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.paysend.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.paysend.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Paysend');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchPaysendQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parsePaysendPayload)(fetchResult.payload, request);
...
exports.runPaysendCollector = runPaysendCollector;
