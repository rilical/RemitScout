...
import { fetchPaysendQuote } from './fetch'
import { extractPaysendMethodPairs, parsePaysendPayload } from './parse'

/**
 * Paysend Collector
 *
 * This collector orchestrates the collection of money transfer quotes from Paysend's API.
...
const logger = createLogger('plane-b.paysend.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractPaysendMethodPairs(payload, corridorId)
...
/**
 * Main collector function that orchestrates quote collection from Paysend.
 *
 ...
 */
export const runPaysendCollector = async (options: PaysendCollectorOptions = {}) => {
  const providerId = 'paysend'
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = PAYSEND_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.paysend.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.paysend.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.paysend.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.paysend.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.paysend.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.paysend.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.paysend.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.paysend.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.paysend.freshnessSloEnabled
  const blockCooldownMs = config.planeB.paysend.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'Paysend')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchPaysendQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchPaysendQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parsePaysendPayload(fetchResult.payload as Record<string, unknown>, request)
