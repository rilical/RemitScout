...
const upsertCapability = async (
  ...
  const pairs = extractWiseMethodPairs(payload)
...
export const runWiseCollector = async (options: WiseCollectorOptions = {}) => {
  ...
  await ensureProvider(pool, providerId, 'Wise')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchWiseQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchWiseQuote(request, { jitterMs, proxyTier })
        ...
        const parsed = parseWisePayload(fetchResult.payload as Record<string, unknown>, request)
        parseDurationMs = Date.now() - parseStartedAt
        if (!parsed) {
          const payload = fetchResult.payload as {
            paymentOptions?: Array<unknown>
            error?: string
            errorCode?: string
            message?: string
            status?: string
