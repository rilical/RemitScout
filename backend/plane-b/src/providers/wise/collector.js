...
exports.runWiseCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.wise.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractWiseMethodPairs)(payload);
 ...
 */
const runWiseCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.WISE_SUPPORTED_CORRIDORS;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Wise');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchWiseQuote)(request, { jitterMs, proxyTier });
                ...
                if (paymentOptions && enabledOptions.length === 0) {
                    ...
                    await (0, base_1.markCorridorUnsupported)(pool, providerId, corridorId, 'auto_no_payment_options');
                    skipCorridor = true;
                    await (0, base_1.insertAttempt)(pool, providerId, {
                        corridorId,
                        amountBucket,
                ...
                const parsed = (0, parse_1.parseWisePayload)(fetchResult.payload, request);
                parseDurationMs = Date.now() - parseStartedAt;
                if (!parsed) {
                    const payload = fetchResult.payload;
                    const optionCount = payload.paymentOptions?.length ?? 0;
                    const errorMessage = payload.error ?? payload.errorCode ?? payload.message ?? payload.status ?? null;
                    logger.warn('quote_parse_failed', {
                        ...
                        payment_option_count: optionCount,
...
exports.runWiseCollector = runWiseCollector;
