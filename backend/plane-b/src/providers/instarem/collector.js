...
exports.runInstaremCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.instarem.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractInstaremMethodPairs)(payload);
 ...
 */
const runInstaremCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.INSTAREM_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.instarem.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.instarem.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.instarem.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.instarem.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.instarem.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.instarem.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.instarem.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.instarem.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.instarem.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.instarem.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'INSTAREM Money Transfer');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchInstaremQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseInstaremPayload)(fetchResult.payload, request);
...
exports.runInstaremCollector = runInstaremCollector;
