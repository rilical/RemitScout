...
import { INSTAREM_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchInstaremQuote } from './fetch'
import { extractInstaremMethodPairs, parseInstaremPayload } from './parse'

/**
 * INSTAREM Collector
 *
 * This collector orchestrates the collection of money transfer quotes from INSTAREM's API.
...
const logger = createLogger('plane-b.instarem.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractInstaremMethodPairs(payload)
...
/**
 * Main collector function that orchestrates quote collection from INSTAREM.
 *
 ...
 */
export const runInstaremCollector = async (options: InstaremCollectorOptions = {}) => {
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = INSTAREM_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.instarem.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.instarem.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.instarem.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.instarem.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.instarem.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.instarem.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.instarem.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.instarem.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.instarem.freshnessSloEnabled
  const blockCooldownMs = config.planeB.instarem.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'INSTAREM Money Transfer')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchInstaremQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchInstaremQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parseInstaremPayload(fetchResult.payload as Record<string, unknown>, request)
