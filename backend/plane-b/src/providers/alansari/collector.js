...
exports.runAlansariCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.alansari.collector');
const upsertCapability = async (pool, corridorId, payload, request) => {
    const pairs = (0, parse_1.extractAlansariMethodPairs)(payload, request);
...
const runAlansariCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.ALANSARI_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.alansari.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.alansari.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.alansari.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.alansari.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.alansari.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.alansari.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.alansari.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.alansari.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.alansari.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.alansari.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Alansari');
    ...
    try {
        for (const corridorId of corridors) {
            ...
            for (const amount of buckets) {
                ...
                try {
                    const response = await (0, fetch_1.fetchAlansariQuote)(request, { proxyTier });
                ...
                const parsed = !attemptErrorType && responsePayload
                    ? (0, parse_1.parseAlansariPayload)(responsePayload, request)
                    : null;
...
exports.runAlansariCollector = runAlansariCollector;
