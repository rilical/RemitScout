 ...
 * Ria Collector
 *
 * This collector orchestrates the collection of money transfer quotes from Ria's API.
...
const upsertCapability = async (
  ...
  const pairs = extractRiaMethodPairs(payload)
 ...
 */
export const runRiaCollector = async (options: RiaCollectorOptions = {}) => {
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = RIA_SUPPORTED_CORRIDORS
  ...
  const jitterMs = options.jitterMs ?? config.planeB.ria.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.ria.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.ria.rateLimitJitterMs
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchRiaQuote>>
        const fetchStartedAt = Date.now()
        // Fetch quote from Ria API
        try {
          fetchResult = await fetchRiaQuote(request, { jitterMs, proxyTier })
