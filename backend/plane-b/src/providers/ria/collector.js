...
exports.runRiaCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.ria.collector');
...
const buildPayinFallbacks = (primary) => {
...
const buildErrorText = (messages, bodyText) => {
    const source = messages.length ? messages.join(' ') : bodyText;
    return source.toLowerCase();
};
const isExplicitUnsupportedCorridor = (text) => CORRIDOR_UNSUPPORTED_PHRASES.some((phrase) => text.includes(phrase));
const isExplicitMethodUnsupported = (text) => METHOD_UNSUPPORTED_PHRASES.some((phrase) => text.includes(phrase));
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractRiaMethodPairs)(payload);
...
/**
 * Main collector function that orchestrates quote collection from Ria.
 *
 ...
 */
const runRiaCollector = async (options = {}) => {
    const providerId = 'ria';
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.RIA_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.ria.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.ria.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.ria.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.ria.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.ria.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.ria.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.ria.corridorJitterMs;
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.ria.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.ria.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.ria.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Ria');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                // Fetch quote from Ria API
                try {
                    fetchResult = await (0, fetch_1.fetchRiaQuote)(request, { jitterMs, proxyTier });
                ...
                if (fetchResult.status !== 200 || !payloadIsObject) {
                    const errorMessages = payloadIsObject
                        ? (0, parse_1.extractRiaErrorMessages)(fetchResult.payload)
                        : [];
                ...
                const parsed = (0, parse_1.parseRiaPayload)(fetchResult.payload, request);
                parseDurationMs = Date.now() - parseStartedAt;
                if (!parsed) {
                    const payload = fetchResult.payload;
                    const errorMessages = (0, parse_1.extractRiaErrorMessages)(payload);
                    const errorText = buildErrorText(errorMessages, fetchResult.bodyText ?? '');
                    const hasQuoteData = Boolean(payload
                        .quote?.individualQuotes?.length) || Boolean(payload
    ...
    await (0, rpm_ramp_1.applyRpmRamp)({
        ...
        rates: providerRates,
        maxRpm: limits_1.httpLimits.rpm,
    });
    if (shouldClose) {
        await pool.end();
    }
...
exports.runRiaCollector = runRiaCollector;
