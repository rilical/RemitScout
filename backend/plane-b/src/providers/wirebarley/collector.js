...
exports.runWireBarleyCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.wirebarley.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractWireBarleyMethodPairs)(payload);
 ...
 */
const runWireBarleyCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.WIREBARLEY_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.wirebarley.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.wirebarley.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.wirebarley.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.wirebarley.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.wirebarley.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.wirebarley.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.wirebarley.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.wirebarley.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.wirebarley.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.wirebarley.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'WireBarley Money Transfer');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchWireBarleyQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseWireBarleyPayload)(fetchResult.payload, request);
...
exports.runWireBarleyCollector = runWireBarleyCollector;
