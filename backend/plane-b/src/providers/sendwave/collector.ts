...
import { SENDWAVE_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchSendwaveQuote } from './fetch'
import { extractSendwaveMethodPairs, getSendwaveErrorMessages, parseSendwavePayload } from './parse'
import type { SendwavePayload } from './parse'

/**
 * Sendwave Collector
 *
 * This collector orchestrates the collection of money transfer quotes from Sendwave's API.
...
const logger = createLogger('plane-b.sendwave.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractSendwaveMethodPairs(capabilityPayload)
...
/**
 * Main collector function that orchestrates quote collection from Sendwave.
 *
 ...
 */
export const runSendwaveCollector = async (options: SendwaveCollectorOptions = {}) => {
  const providerId = 'sendwave'
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = SENDWAVE_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.sendwave.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.sendwave.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.sendwave.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.sendwave.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.sendwave.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.sendwave.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.sendwave.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.sendwave.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.sendwave.freshnessSloEnabled
  const blockCooldownMs = config.planeB.sendwave.blockCooldownMs
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchSendwaveQuote>>
        const fetchStartedAt = Date.now()
        // Fetch quote from Sendwave API
        try {
          fetchResult = await fetchSendwaveQuote(request, { jitterMs, proxyTier })
        ...
        const rawPayload = fetchResult.payload ?? fetchResult.bodyText
        ...
        const errorMessages = getSendwaveErrorMessages(fetchResult.payload as SendwavePayload)
        const hasInvalidCorridor = errorMessages.some((message) => {
          const normalized = message.toLowerCase()
          return normalized.includes('invalid iso2') || normalized.includes('must be one of')
        })
        ...
        if (fetchResult.status !== 200 || !fetchResult.payload || typeof fetchResult.payload !== 'object') {
          ...
          // Mark corridor as unsupported if invalid corridor errors are returned.
          const unsupportedCorridor = fetchResult.status === 422 || hasInvalidCorridor
          if (unsupportedCorridor) {
            await markCorridorUnsupported(pool, providerId, corridorId, 'auto_invalid_corridor')
        ...
        const sendwavePayload = fetchResult.payload as SendwavePayload
        const payoutGroups = sendwavePayload?.segments && typeof sendwavePayload.segments === 'object'
          ? (sendwavePayload.segments as { payoutMethodsAndPrices?: unknown[] }).payoutMethodsAndPrices
          : null
        const hasNoPayoutMethods = Array.isArray(payoutGroups) && payoutGroups.length === 0

        if (hasInvalidCorridor || hasNoPayoutMethods) {
          const reason = hasNoPayoutMethods ? 'auto_no_payout_methods' : 'auto_invalid_corridor'
          await markCorridorUnsupported(pool, providerId, corridorId, reason)
          logger.warn('corridor_marked_unsupported', {
            trace_id: traceId,
            corridor_id: corridorId,
            provider_id: providerId,
        ...
        const parsed = parseSendwavePayload(fetchResult.payload as Record<string, unknown>, request)
