...
exports.runSendwaveCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.sendwave.collector');
/**
 ...
 */
const upsertCapability = async (pool, corridorId, capabilityPayload) => {
    const pairs = (0, parse_1.extractSendwaveMethodPairs)(capabilityPayload);
 ...
 */
const runSendwaveCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.SENDWAVE_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.sendwave.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.sendwave.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.sendwave.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.sendwave.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.sendwave.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.sendwave.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.sendwave.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.sendwave.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.sendwave.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.sendwave.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Sendwave');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchSendwaveQuote)(request, { jitterMs, proxyTier });
                ...
                const errorMessages = (0, parse_1.getSendwaveErrorMessages)(fetchResult.payload);
                ...
                if (fetchResult.status !== 200 || !fetchResult.payload || typeof fetchResult.payload !== 'object') {
                    ...
                    if (unsupportedCorridor) {
                        await (0, base_1.markCorridorUnsupported)(pool, providerId, corridorId, 'auto_invalid_corridor');
                ...
                const sendwavePayload = fetchResult.payload;
                const payoutGroups = sendwavePayload?.segments && typeof sendwavePayload.segments === 'object'
                    ? sendwavePayload.segments.payoutMethodsAndPrices
                    : null;
                ...
                if (hasInvalidCorridor || hasNoPayoutMethods) {
                    ...
                    await (0, base_1.markCorridorUnsupported)(pool, providerId, corridorId, reason);
                    logger.warn('corridor_marked_unsupported', {
                        trace_id: traceId,
                        corridor_id: corridorId,
                        provider_id: providerId,
                ...
                const parsed = (0, parse_1.parseSendwavePayload)(fetchResult.payload, request);
...
exports.runSendwaveCollector = runSendwaveCollector;
