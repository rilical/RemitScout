...
import { PANGEA_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchPangeaQuote } from './fetch'
import type { PangeaPayload } from './parse'
import { extractPangeaMethodPairs, parsePangeaPayload } from './parse'
...
const logger = createLogger('plane-b.pangea.collector')
...
export const runPangeaCollector = async (options: PangeaCollectorOptions = {}) => {
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = PANGEA_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.pangea.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.pangea.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.pangea.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.pangea.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.pangea.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.pangea.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.pangea.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.pangea.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.pangea.freshnessSloEnabled
  const blockCooldownMs = config.planeB.pangea.blockCooldownMs
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchPangeaQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchPangeaQuote(request, { jitterMs, proxyTier })
        ...
        const parsed = parsePangeaPayload(fetchResult.payload as Record<string, unknown>, request)
