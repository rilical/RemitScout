...
exports.runPangeaCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.pangea.collector');
const upsertCapability = async (pool, corridorId, _payload) => {
    const pairs = (0, parse_1.extractPangeaMethodPairs)();
...
const runPangeaCollector = async (options = {}) => {
    const providerId = 'pangea';
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.PANGEA_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.pangea.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.pangea.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.pangea.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.pangea.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.pangea.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.pangea.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.pangea.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.pangea.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.pangea.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.pangea.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Pangea');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchPangeaQuote)(request, { jitterMs, proxyTier });
                ...
                const rateLimited = isRateLimit(reason);
                if (blockResult.blocked) {
                    ...
                    if (alertId) {
                        await (0, alert_routing_1.notifyBlockAlert)(pool, alertId, { force: true });
                    }
                    ...
                    await (0, base_1.pauseProviderForBlock)(pool, providerId, corridorId, reason ?? 'blocked', blockCooldownMs);
                    blocked = true;
                    blockReason = reason;
                    const attemptDurationMs = recordAttemptDuration(attemptStartedAt);
                    logger.info('quote_attempt_finish', {
                ...
                const parsed = (0, parse_1.parsePangeaPayload)(fetchResult.payload, request);
...
exports.runPangeaCollector = runPangeaCollector;
