...
import { MUKURU_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchMukuruQuote } from './fetch'
import { extractMukuruMethodPairs, getMukuruErrorMessages, parseMukuruPayload } from './parse'
import type { MukuruPayload } from './fetch'

/**
 * Mukuru Collector
 *
 * This collector orchestrates the collection of money transfer quotes from Mukuru's API.
...
const logger = createLogger('plane-b.mukuru.collector')
...
/**
 * Main collector function that orchestrates quote collection from Mukuru.
 *
 ...
 */
export const runMukuruCollector = async (options: MukuruCollectorOptions = {}) => {
  const providerId = 'mukuru'
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = MUKURU_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.mukuru.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.mukuru.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.mukuru.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.mukuru.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.mukuru.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.mukuru.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.mukuru.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.mukuru.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.mukuru.freshnessSloEnabled
  const blockCooldownMs = config.planeB.mukuru.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'Mukuru')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchMukuruQuote>>
        const fetchStartedAt = Date.now()
        // Fetch quote from Mukuru API
        try {
          fetchResult = await fetchMukuruQuote(request, { jitterMs, proxyTier })
        ...
        const errorMessages = getMukuruErrorMessages(fetchResult.payload as MukuruPayload)
        ...
        const mukuruPayload = fetchResult.payload as MukuruPayload
        const hasNoProducts = Array.isArray(mukuruPayload.products) && mukuruPayload.products.length === 0

        if (hasInvalidCorridor || hasNoProducts) {
          const reason = hasNoProducts ? 'auto_no_products' : 'auto_invalid_corridor'
        ...
        if (!capabilityUpdated.has(corridorId)) {
          await upsertCapability(pool, corridorId, mukuruPayload)
          capabilityUpdated.add(corridorId)
        }

        // Parse provider-specific payload format to standard parsed quote
        const parseStartedAt = Date.now()
        const parsed = parseMukuruPayload(mukuruPayload, request)
