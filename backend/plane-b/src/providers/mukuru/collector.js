...
exports.runMukuruCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.mukuru.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractMukuruMethodPairs)(payload);
 ...
 */
const runMukuruCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.MUKURU_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.mukuru.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.mukuru.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.mukuru.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.mukuru.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.mukuru.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.mukuru.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.mukuru.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.mukuru.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.mukuru.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.mukuru.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Mukuru');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchMukuruQuote)(request, { jitterMs, proxyTier });
                ...
                const errorMessages = (0, parse_1.getMukuruErrorMessages)(fetchResult.payload);
                ...
                const mukuruPayload = fetchResult.payload;
                ...
                const parsed = (0, parse_1.parseMukuruPayload)(mukuruPayload, request);
...
exports.runMukuruCollector = runMukuruCollector;
