...
export const MUKURU_SOURCE_COUNTRIES: SourceCountry[] = [
...
const MUKURU_DESTINATION_PAIRS: Array<[string, string]> = [
  ['AO', 'AOA'],
  ['AT', 'EUR'],
  ['BD', 'BDT'],
  ['BE', 'EUR'],
...
const buildSupportedCorridors = () => {
  ...
  for (const source of MUKURU_SOURCE_COUNTRIES) {
    for (const [destCountry, destCurrency] of MUKURU_DESTINATION_PAIRS) {
      if (source.code === destCountry) continue
...
export const MUKURU_SUPPORTED_CORRIDORS = buildSupportedCorridors()
