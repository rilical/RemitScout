...
import { BOSSMONEY_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchBossMoneyQuote } from './fetch'
import { extractBossMoneyMethodPairs, parseBossMoneyPayload } from './parse'

/**
 * BossMoney Collector
 *
 * This collector orchestrates the collection of money transfer quotes from BossMoney's API.
 ...
 *    - Fetch quote from BossMoney API
...
type BossMoneyCollectorOptions = {
  pool?: Pool
  closePool?: boolean
  corridors?: string[]
  delayMs?: number
...
const logger = createLogger('plane-b.bossmoney.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractBossMoneyMethodPairs(payload)
...
/**
 * Main collector function that orchestrates quote collection from BossMoney.
 *
 ...
 */
export const runBossMoneyCollector = async (options: BossMoneyCollectorOptions = {}) => {
  const providerId = 'bossmoney'
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = BOSSMONEY_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.bossmoney.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.bossmoney.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.bossmoney.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.bossmoney.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.bossmoney.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.bossmoney.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.bossmoney.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.bossmoney.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.bossmoney.freshnessSloEnabled
  const blockCooldownMs = config.planeB.bossmoney.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'Boss Money')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchBossMoneyQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchBossMoneyQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parseBossMoneyPayload(fetchResult.payload as Record<string, unknown>, request)
