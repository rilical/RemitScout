...
exports.runBossMoneyCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.bossmoney.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractBossMoneyMethodPairs)(payload);
 ...
 */
const runBossMoneyCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.BOSSMONEY_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.bossmoney.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.bossmoney.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.bossmoney.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.bossmoney.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.bossmoney.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.bossmoney.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.bossmoney.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.bossmoney.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.bossmoney.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.bossmoney.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'Boss Money');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchBossMoneyQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseBossMoneyPayload)(fetchResult.payload, request);
...
exports.runBossMoneyCollector = runBossMoneyCollector;
