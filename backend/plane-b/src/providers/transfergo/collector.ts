...
import { fetchTransferGoQuote } from './fetch'
import { extractTransferGoMethodPairs, parseTransferGoPayload } from './parse'

/**
 * TransferGo Collector
 *
 * This collector orchestrates the collection of money transfer quotes from TransferGo's API.
 ...
 *    - Fetch quote from TransferGo API
...
type TransferGoCollectorOptions = {
  pool?: Pool
  closePool?: boolean
  corridors?: string[]
  delayMs?: number
...
const logger = createLogger('plane-b.transfergo.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractTransferGoMethodPairs(payload)
...
/**
 * Main collector function that orchestrates quote collection from TransferGo.
 *
 ...
 */
export const runTransferGoCollector = async (options: TransferGoCollectorOptions = {}) => {
  const providerId = 'transfergo'
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = TRANSFERGO_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.transfergo.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.transfergo.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.transfergo.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.transfergo.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.transfergo.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.transfergo.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.transfergo.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.transfergo.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.transfergo.freshnessSloEnabled
  const blockCooldownMs = config.planeB.transfergo.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'TransferGo')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchTransferGoQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchTransferGoQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parseTransferGoPayload(fetchResult.payload as Record<string, unknown>, request)
