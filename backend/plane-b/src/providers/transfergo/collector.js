...
exports.runTransferGoCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.transfergo.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractTransferGoMethodPairs)(payload);
 ...
 */
const runTransferGoCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.TRANSFERGO_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.transfergo.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.transfergo.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.transfergo.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.transfergo.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.transfergo.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.transfergo.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.transfergo.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.transfergo.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.transfergo.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.transfergo.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'TransferGo');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchTransferGoQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseTransferGoPayload)(fetchResult.payload, request);
...
exports.runTransferGoCollector = runTransferGoCollector;
