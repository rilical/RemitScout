...
exports.runWorldRemitCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.worldremit.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractWorldRemitMethodPairs)(payload);
    ...
    if (!payinMethods.length && !payoutMethods.length) {
        await (0, base_1.markCorridorUnsupported)(pool, 'worldremit', corridorId, 'auto_empty_methods');
        return;
    }
 ...
 */
const runWorldRemitCollector = async (options = {}) => {
    ...
    const candidateCorridors = options.corridors?.length
        ? options.corridors
        : supported_corridors_1.WORLDREMIT_SUPPORTED_CORRIDORS;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'WorldRemit');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchWorldRemitQuote)(request, { jitterMs, proxyTier });
                ...
                const responsePayload = fetchResult.payload;
                ...
                if (Array.isArray(payoutMethods) && payoutMethods.length === 0) {
                    ...
                    await (0, base_1.markCorridorUnsupported)(pool, providerId, corridorId, 'auto_no_payout_methods');
                    unsupportedCorridors.add(corridorId);
                    skipCorridor = true;
                    await (0, base_1.insertAttempt)(pool, providerId, {
                        corridorId,
                ...
                const parsed = (0, parse_1.parseWorldRemitPayload)(fetchResult.payload, request);
                ...
                if (!parsed) {
                    ...
                    const shouldMarkUnsupported = payoutCount === 0
                        || payinCount === 0
...
exports.runWorldRemitCollector = runWorldRemitCollector;
