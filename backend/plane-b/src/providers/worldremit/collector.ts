...
import { fetchWorldRemitQuote } from './fetch'
import { extractWorldRemitMethodPairs, parseWorldRemitPayload } from './parse'

/**
 * WorldRemit Collector
 *
 * This collector orchestrates the collection of money transfer quotes from WorldRemit's GraphQL API.
 ...
 *    - Fetch quote from WorldRemit GraphQL API (may require multiple calls)
...
type WorldRemitCollectorOptions = {
  pool?: Pool
  corridors?: string[]
  delayMs?: number
  jitterMs?: number
...
const logger = createLogger('plane-b.worldremit.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractWorldRemitMethodPairs(payload)
...
/**
 * Main collector function that orchestrates quote collection from WorldRemit.
 *
...
export const runWorldRemitCollector = async (options: WorldRemitCollectorOptions = {}) => {
  ...
  const delayMs = options.delayMs ?? config.planeB.worldremit.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.worldremit.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.worldremit.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.worldremit.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.worldremit.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.worldremit.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.worldremit.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.worldremit.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.worldremit.freshnessSloEnabled
  const blockCooldownMs = config.planeB.worldremit.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'WorldRemit')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchWorldRemitQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchWorldRemitQuote(request, { jitterMs, proxyTier })
        ...
        const parsed = parseWorldRemitPayload(fetchResult.payload as Record<string, unknown>, request)
        parseDurationMs = Date.now() - parseStartedAt
        if (!parsed) {
          const payload = fetchResult.payload as {
            payoutMethods?: { payOutMethods?: Array<unknown> }
            calculation?: { createCalculation?: { calculation?: { payInMethodsCalculations?: Array<unknown> }; errors?: Array<{ message?: string }> } }
          }
          ...
          logger.warn('quote_parse_failed', {
            ...
            payout_method_count: payoutCount,
