...
exports.runOrbitRemitCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.orbitremit.collector');
/**
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractOrbitRemitMethodPairs)(payload);
 ...
 */
const runOrbitRemitCollector = async (options = {}) => {
    ...
    if (options.corridors?.length) {
        ...
        const allPossibleCorridors = supported_corridors_1.ORBITREMIT_SUPPORTED_CORRIDORS;
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.orbitremit.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.orbitremit.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.orbitremit.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.orbitremit.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.orbitremit.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.orbitremit.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.orbitremit.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.orbitremit.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.orbitremit.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.orbitremit.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'OrbitRemit');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchOrbitRemitQuote)(request, {
                        jitterMs,
                        proxyTier,
                    });
                ...
                const parsed = (0, parse_1.parseOrbitRemitPayload)(fetchResult.payload, request);
...
exports.runOrbitRemitCollector = runOrbitRemitCollector;
