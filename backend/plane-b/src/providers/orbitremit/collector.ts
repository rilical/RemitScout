...
import { ORBITREMIT_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchOrbitRemitQuote } from './fetch'
import { extractOrbitRemitMethodPairs, parseOrbitRemitPayload } from './parse'

/**
 * OrbitRemit Collector
 *
 * This collector orchestrates the collection of money transfer quotes from OrbitRemit's API.
 ...
 *    - Fetch quote from OrbitRemit API
...
type OrbitRemitCollectorOptions = {
  pool?: Pool
  closePool?: boolean
  corridors?: string[]
  delayMs?: number
...
const logger = createLogger('plane-b.orbitremit.collector')
...
const upsertCapability = async (
  ...
  const pairs = extractOrbitRemitMethodPairs(payload)
...
/**
 * Main collector function that orchestrates quote collection from OrbitRemit.
 *
 ...
 */
export const runOrbitRemitCollector = async (options: OrbitRemitCollectorOptions = {}) => {
  const providerId = 'orbitremit'
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = ORBITREMIT_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.orbitremit.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.orbitremit.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.orbitremit.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.orbitremit.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.orbitremit.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.orbitremit.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.orbitremit.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.orbitremit.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.orbitremit.freshnessSloEnabled
  const blockCooldownMs = config.planeB.orbitremit.blockCooldownMs
  ...
  await ensureProvider(pool, providerId, 'OrbitRemit')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchOrbitRemitQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchOrbitRemitQuote(request, {
            jitterMs,
            proxyTier,
          })
        ...
        const parsed = parseOrbitRemitPayload(fetchResult.payload as Record<string, unknown>, request)
