...
import { DAHABSHIIL_SUPPORTED_CORRIDORS } from './supported-corridors'
import { httpLimits } from './limits'
import { fetchDahabshiilQuote } from './fetch'
import { extractDahabshiilMethodPairs, getDahabshiilErrorMessages, parseDahabshiilPayload } from './parse'
import type { DahabshiilPayload } from './parse'

/**
 * Dahabshiil Collector
 *
 * This collector orchestrates the collection of money transfer quotes from Dahabshiil's API.
...
type DahabshiilCollectorOptions = {
  pool?: Pool
  corridors?: string[]
  delayMs?: number
  jitterMs?: number
...
const logger = createLogger('plane-b.dahabshiil.collector')
 ...
 */
export const runDahabshiilCollector = async (options: DahabshiilCollectorOptions = {}) => {
  ...
  if (options.corridors?.length) {
    ...
    const allPossibleCorridors = DAHABSHIIL_SUPPORTED_CORRIDORS
  ...
  const delayMs = options.delayMs ?? config.planeB.dahabshiil.delayMs
  const jitterMs = options.jitterMs ?? config.planeB.dahabshiil.jitterMs
  const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config.planeB.dahabshiil.rateLimitBackoffMs
  const rateLimitJitterMs = options.rateLimitJitterMs ?? config.planeB.dahabshiil.rateLimitJitterMs
  const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config.planeB.dahabshiil.rateLimitMaxRetries
  const corridorDelayMs = options.corridorDelayMs ?? config.planeB.dahabshiil.corridorDelayMs
  const corridorJitterMs = options.corridorJitterMs ?? config.planeB.dahabshiil.corridorJitterMs
  const collectorType = options.collectorType ?? 'collector'
  const freshnessSloMinutes = options.freshnessSloMinutes ?? config.planeB.dahabshiil.freshnessSloMinutes
  const freshnessSloEnabled = options.freshnessSloEnabled ?? config.planeB.dahabshiil.freshnessSloEnabled
  const blockCooldownMs = config.planeB.dahabshiil.blockCooldownMs
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchDahabshiilQuote>>
        const fetchStartedAt = Date.now()
        // Fetch quote from Dahabshiil API
        try {
          fetchResult = await fetchDahabshiilQuote(request, { jitterMs, proxyTier })
        ...
        const errorMessages = getDahabshiilErrorMessages(fetchResult.payload as DahabshiilPayload)
        const hasUnsupportedFx = errorMessages.some(message =>
          message.toLowerCase().includes('no valid fx rate'),
        )
        if (hasUnsupportedFx) {
          await markCorridorUnsupported(pool, providerId, corridorId, 'auto_fx_missing')
          logger.warn('corridor_marked_unsupported', {
            trace_id: traceId,
            corridor_id: corridorId,
            provider_id: providerId,
        ...
        const parsed = parseDahabshiilPayload(fetchResult.payload as Record<string, unknown>, request)
