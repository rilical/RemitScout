...
exports.runKoronaPayCollector = void 0;
...
const logger = (0, logger_1.createLogger)('plane-b.koronapay.collector');
const isKoronaPayErrorPayload = (value) => {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const candidate = value;
    return Boolean(candidate.type || candidate.code || candidate.message);
};
const shouldMarkKoronaPayUnsupported = (payload) => {
...
const upsertCapability = async (pool, corridorId, payload) => {
    const pairs = (0, parse_1.extractKoronaPayMethodPairs)(payload);
 ...
 */
const runKoronaPayCollector = async (options = {}) => {
    ...
    const candidateCorridors = options.corridors?.length
        ? options.corridors
        : supported_corridors_1.KORONAPAY_SUPPORTED_CORRIDORS;
    const corridors = candidateCorridors.filter(corridor => !unsupportedCorridors.has(corridor));
    ...
    const delayMs = options.delayMs ?? config_1.config.planeB.koronapay.delayMs;
    const jitterMs = options.jitterMs ?? config_1.config.planeB.koronapay.jitterMs;
    const rateLimitBackoffMs = options.rateLimitBackoffMs ?? config_1.config.planeB.koronapay.rateLimitBackoffMs;
    const rateLimitJitterMs = options.rateLimitJitterMs ?? config_1.config.planeB.koronapay.rateLimitJitterMs;
    const rateLimitMaxRetries = options.rateLimitMaxRetries ?? config_1.config.planeB.koronapay.rateLimitMaxRetries;
    const corridorDelayMs = options.corridorDelayMs ?? config_1.config.planeB.koronapay.corridorDelayMs;
    const corridorJitterMs = options.corridorJitterMs ?? config_1.config.planeB.koronapay.corridorJitterMs;
    const collectorType = options.collectorType ?? 'collector';
    const freshnessSloMinutes = options.freshnessSloMinutes ?? config_1.config.planeB.koronapay.freshnessSloMinutes;
    const freshnessSloEnabled = options.freshnessSloEnabled ?? config_1.config.planeB.koronapay.freshnessSloEnabled;
    const blockCooldownMs = config_1.config.planeB.koronapay.blockCooldownMs;
    ...
    await (0, base_1.ensureProvider)(pool, providerId, 'KoronaPay');
    ...
    for (const corridorId of corridors) {
        ...
        for (const amountBucket of buckets) {
            ...
            while (!completed) {
                ...
                try {
                    fetchResult = await (0, fetch_1.fetchKoronaPayQuote)(request, { jitterMs, proxyTier });
                ...
                const parsed = (0, parse_1.parseKoronaPayPayload)(fetchResult.payload, request);
                ...
                if (!parsed) {
                    ...
                    if (shouldMarkUnsupported) {
                        await (0, base_1.markCorridorUnsupported)(pool, providerId, corridorId, 'auto_empty_tariffs');
                        unsupportedCorridors.add(corridorId);
                        skipCorridor = true;
                    }
                    await (0, base_1.insertAttempt)(pool, providerId, {
...
exports.runKoronaPayCollector = runKoronaPayCollector;
