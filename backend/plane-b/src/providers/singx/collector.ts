...
export const runSingxCollector = async (options: SingxCollectorOptions = {}) => {
  ...
  await ensureProvider(pool, providerId, 'SingX')
  ...
  for (const corridorId of corridors) {
    ...
    for (const amountBucket of buckets) {
      ...
      while (!completed) {
        ...
        let fetchResult: Awaited<ReturnType<typeof fetchSingxQuote>>
        const fetchStartedAt = Date.now()
        try {
          fetchResult = await fetchSingxQuote(request, { jitterMs, proxyTier })
        ...
        const parsed = parseSingxPayload(fetchResult.payload as Record<string, unknown>, request)
