...
export class WebhookRepository implements IWebhookRepository {
  ...
  async loadActiveSubscriptions(
    ...
    try {
      ...
      const fallback = await query<WebhookSubscriptionRecord>(
        `SELECT subscription_id, client_id, webhook_url, NULL::text AS webhook_secret, corridor_filter, provider_filter, z_score_threshold
           ${whereClause}`,
        params,
        this.pool,
      )
