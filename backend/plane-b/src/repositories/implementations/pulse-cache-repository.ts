...
import {
  ...
  resolvePulseInterval,
  resolvePulseRangeInterval,
  resolvePulseBucket,
} from '../../../../shared/pulse-cache-keys'
import { pulseCache } from '../../../../shared/repository-cache'
...
const logger = createLogger('repo.pulse-cache')
...
const buildWinnerHistory = (providerDaily: Array<Record<string, unknown>>) => {
  ...
  return Array.from(winnerCandidatesByDay.entries())
...
const pushParam = (params: Array<string | number>) => {
...
const buildCorridorConditions = (
  filters: PulseCacheFilters,
  params: Array<string | number>,
) => {
  const corridor = normalizePulseCorridor(filters.corridor)
  if (!corridor) return []
...
const buildQuoteFilters = (
  filters: PulseCacheFilters,
  alias: string,
  params: Array<string | number>,
  options: {
    includeAmount?: boolean
    includePayin?: boolean
  ...
  if (includeTimeframe) {
    const interval = options.intervalOverride ?? resolvePulseInterval(filters.timeframe)
    const addParam = pushParam(params)
  ...
  if (options.includePayin !== false) {
    const payin = normalizePulseMethod(filters.payin)
    if (payin) {
      const addParam = pushParam(params)
  ...
  if (options.includePayout !== false) {
    const payout = normalizePulseMethod(filters.payout)
    if (payout) {
      const addParam = pushParam(params)
...
const buildChartSeries = (
  ...
  const buildLine = (id: string, label: string, color: string, values: number[]) => ({
    id,
    label,
    color,
  ...
  for (const entry of winnerHistory) {
    providerWins.set(entry.winner, (providerWins.get(entry.winner) ?? 0) + 1)
  }

  const providerOrder = Array.from(
    new Set(providerDaily.map((row) => String(row.provider_name || row.provider_id || 'Unknown'))),
  ).sort((left, right) => {
    const winDiff = (providerWins.get(right) ?? 0) - (providerWins.get(left) ?? 0)
  ...
  switch (chartId) {
    case 'all-in-cost': {
      const values = dailyMap.map((row) => {
        if (!row.best_rate || amount <= 0) return 0
    ...
    case 'provider-winner': {
      for (const provider of providerOrder) {
        series.push({
          ...
          points: winnerHistory.map((entry) => ({
            t: entry.timestamp,
            v: entry.winner === provider ? 1 : 0,
            ...(entry.winner === provider
              ? {
                  label: [
                    `Winner: ${entry.winner}`,
                    `Recipient gets: ${entry.winnerValue.toFixed(2)}`,
                    entry.runnerUp
                      ? `Edge vs #2 (${entry.runnerUp}): ${entry.edgeAmount.toFixed(2)} (${entry.edgeBps.toFixed(1)} bps)`
                      : 'Only tracked provider on this publication',
                  ].join(' · '),
                }
              : {}),
          })),
    ...
    case 'leader-change-frequency': {
      series.push({
        ...
        color: '#2563eb',
        points: winnerHistory.map((entry, index) => {
          const previous = winnerHistory[index - 1]
          const changed = Boolean(previous && previous.winner !== entry.winner)
          return {
            t: entry.timestamp,
            v: changed ? 1 : 0,
            label: changed
              ? `Leader flipped from ${previous?.winner ?? '—'} to ${entry.winner}`
              : `Leader held steady: ${entry.winner}`,
          }
        }),
      })
      break
    ...
    case 'leader-edge': {
      series.push({
        ...
        points: winnerHistory.map((entry) => ({
          t: entry.timestamp,
          v: entry.edgeBps,
          label: entry.runnerUp
            ? `${entry.winner} led ${entry.runnerUp} by ${entry.edgeBps.toFixed(1)} bps`
            : `${entry.winner} was the only contributing provider`,
        })),
      })
      break
    }
    ...
    case 'volatility-pulse': {
      const values = dailyMap.map((row, index) => {
        ...
        if (!previous || !previous.best_rate) return 0
    ...
    case 'quote-anomalies': {
      const grouped = new Map<string, { provider: string; color: string; points: Array<{ t: number; v: number }> }>()
      for (const row of providerDaily) {
        const provider = String(row.provider_name || row.provider_id || 'Unknown')
    ...
    case 'quote-success': {
      ...
      const times = quoteAttempts.map((row) => new Date(row.bucket as Date).getTime())
      series.push({
        id: 'success',
        label: 'Success rate',
        color: '#2563eb',
...
export class PulseCacheRepository implements IPulseCacheRepository {
  ...
  async upsertEntry(input: PulseCacheEntryInput): Promise<void> {
    ...
    try {
      await withCircuitBreaker('pulse-cache', async () => {
        await withRetry(async () => {
          ...
          await pulseCache.invalidate(input.key)
        })
      })
      ...
      await recordRepositoryMetric('pulse-cache', 'upsert', durationMs, success, errorType)
    }
  }

  async listPulseCorridors(): Promise<PulseCorridorFilter[]> {
    const interval = resolvePulseInterval('1y')
  ...
  async listPulseMethods(filters: PulseCacheFilters): Promise<PulseMethodFilter[]> {
    ...
    const result = await query<{ payin: string; payout: string }>(
  ...
  async aggregatePulseCacheData(filters: PulseCacheFilters): Promise<Map<string, unknown>> {
    const startTime = Date.now()
    const cacheKey = buildPulseCacheKey('aggregate', filters)
    let success = false
    let errorType: string | undefined
    ...
    try {
      ...
      const rangeInterval = resolvePulseRangeInterval(filters.range)
      const bucketUnit = resolvePulseBucket(rangeInterval)
      ...
      const methodParams: Array<string | number> = []
      ...
      const overviewParams: Array<string | number> = []
      const addOverviewParam = pushParam(overviewParams)
      addOverviewParam(resolvePulseInterval(filters.timeframe))
      ...
      const latestQuotes = latestQuotesResult.rows as Array<Record<string, unknown>>
      const dailyStats = dailyStatsResult.rows as Array<Record<string, unknown>>
      const providerDaily = providerDailyResult.rows as Array<Record<string, unknown>>
      const methodCoverageRows = methodCoverageResult.rows as Array<Record<string, unknown>>
      const overviewRow = overviewResult.rows[0] as Record<string, unknown> | undefined
      const coverageRow = coverageResult.rows[0] as Record<string, unknown> | undefined
      const snapshotRow = snapshotResult.rows[0] as Record<string, unknown> | undefined
      const benchmarkingRows = benchmarkingResult.rows as Array<Record<string, unknown>>
      const eventRows = eventResult.rows as Array<Record<string, unknown>>
      const attemptRows = attemptResult.rows as Array<Record<string, unknown>>
      const reliabilityRows = reliabilityResult.rows as Array<Record<string, unknown>>
      ...
      const methodCoverage = methodCoverageRows.map((row) => ({
        ...
        speed: formatSpeed(
          row.min_delivery !== null ? toNumber(row.min_delivery, 0) : null,
          row.max_delivery !== null ? toNumber(row.max_delivery, 0) : null,
        ),
      }))
      ...
      for (const entry of winnerHistory) {
        ...
        providerHeatmapDays.push({
          ...
          winner: entry.winner,
          winnerColor: getProviderColor(entry.winner),
          savings: entry.edgeAmount,
        })
        providerWinCounts[entry.winner] = (providerWinCounts[entry.winner] || 0) + 1
      }
      ...
      const marketSnapshotQuotes = latestQuotes.map((row) => {
        ...
        return {
          ...
          markupBps: 0,
          speed: formatSpeed(
            row.delivery_time_min_minutes !== null ? toNumber(row.delivery_time_min_minutes, 0) : null,
            row.delivery_time_max_minutes !== null ? toNumber(row.delivery_time_max_minutes, 0) : null,
          ),
      ...
      const trueCost = updatedQuotes.map((quote) => {
        ...
        return {
          ...
          methods: [normalizePulseMethod(filters.payin) || 'bank', normalizePulseMethod(filters.payout) || 'bank'].filter(Boolean),
      ...
      const costTrend = dailyStats.map((row) => {
        const timestamp = new Date(row.bucket as Date).toISOString().slice(0, 10)
      ...
      const smartSend = (() => {
        ...
        if (percentFromAvg >= 1.0) level = 'great'
        else if (percentFromAvg >= 0.2) level = 'good'
        else if (percentFromAvg <= -0.5) level = 'wait'

        const messageMap: Record<typeof level, string> = {
      ...
      const hero = (() => {
        const points = dailyStats.map((row) => {
          ...
          return {
            timestamp: new Date(row.bucket as Date).getTime(),
      ...
      const fxRateHistoryPayload = await (async () => {
        const corridorSlug = normalizePulseCorridor(filters.corridor)
        const corridorPair = parseCorridorSlug(corridorSlug)
        if (!corridorPair) return null
        ...
        try {
          ...
          const historyRows = await historyRepository.getLatestHistory(
            baseCurrency,
            quoteCurrency,
            historyDays,
          )
      ...
      const durationMs = Date.now() - startTime
      await recordRepositoryMetric('pulse-cache', 'aggregate', durationMs, success, errorType)
    }
  }

  async getEntry(key: string): Promise<unknown | null> {
    const startTime = Date.now()
    let success = false
    let errorType: string | undefined

    try {
      const cached = await pulseCache.get(key)
      if (cached !== null) {
        return cached
      }

      const result = await withCircuitBreaker('pulse-cache', async () => {
        return await withRetry(async () => {
          return await query<{ payload: string }>(
            `SELECT payload FROM gold.pulse_cache WHERE key = $1`,
            [key],
            this.pool,
          )
        })
      ...
      await pulseCache.set(key, parsed)
      success = true
      return parsed
    } catch (error: unknown) {
      const { message } = formatError(error)
      ...
      const durationMs = Date.now() - startTime
      await recordRepositoryMetric('pulse-cache', 'get', durationMs, success, errorType)
    }
  }

  async invalidateEntry(key: string): Promise<void> {
    try {
      await pulseCache.invalidate(key)
