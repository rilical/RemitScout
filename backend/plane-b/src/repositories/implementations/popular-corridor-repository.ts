...
import type {
  ...
  PopularCorridorAggregationRow,
  PopularCorridorInput,
...
export class PopularCorridorRepository implements IPopularCorridorRepository {
  ...
  async clearAll(): Promise<void> {
    await query('DELETE FROM gold.popular_corridors', [], this.pool)
  }

  async insertCorridor(input: PopularCorridorInput): Promise<void> {
    await query(
  ...
  async aggregatePopularCorridors(maxResults: number = 100): Promise<PopularCorridorAggregationRow[]> {
    const result = await query<PopularCorridorAggregationRow>(
