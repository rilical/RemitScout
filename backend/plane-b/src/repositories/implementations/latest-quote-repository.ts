...
import type {
  ...
  LatestQuoteAgeRecord,
  LatestQuoteUpsertInput,
...
export class LatestQuoteRepository implements ILatestQuoteRepository {
  ...
  async getLatestCollectedAt(
    ...
    const result = await query<{ collected_at: Date | null }>(
      `SELECT collected_at
  ...
  async getLatestQuoteAgeMinutes(
    providerId: string,
    corridorId: string,
    amountBucket: number,
    payinMethod: string,
  ...
  async loadFreshnessLagByCorridor(
    providerId: string,
    corridors: string[],
    amountBucket: number,
    payinMethod: string,
    payoutMethod: string,
  ): Promise<LatestQuoteAgeRecord[]> {
    const result = await query<LatestQuoteAgeRecord>(
      `SELECT corridor_id,
  ...
  async upsertLatestQuote(input: LatestQuoteUpsertInput): Promise<void> {
    await query(
