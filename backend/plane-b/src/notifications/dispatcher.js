...
exports.dispatchQueuedSignal = exports.dispatchSignal = void 0;
const crypto_1 = require("crypto");
const promises_1 = require("timers/promises");
const logger_1 = require("../../../shared/logger");
const config_1 = require("../../../shared/config");
const sqs_1 = require("../../../shared/sqs");
const worker_retry_1 = require("../../../shared/worker-retry");
const repositories_1 = require("../repositories");
const config_2 = require("./config");
const aws_services_1 = require("./aws-services");
const logger = (0, logger_1.createLogger)('plane-b.notifications.dispatcher');
const notificationsQueueUrl = config_1.config.queues.notifications.url;
const notificationsQueueMode = config_1.config.queues.notifications.mode;
let notifiedQueueMisconfig = false;
const normalizeWebhookSignalType = (value) => {
...
const enqueueNotification = async (payload) => {
    ...
    try {
        await (0, worker_retry_1.withWorkerRetry)(() => (0, sqs_1.sendJsonMessage)(notificationsQueueUrl, payload), { maxRetries: 2, initialDelayMs: 500, maxDelayMs: 10000 });
        return true;
    }
    catch (error) {
...
const dispatchWebhookBatch = async (subscriptions, payload) => {
    if (!subscriptions.length)
        return;
    if (config_2.NOTIFICATION_CONFIG.PARALLEL_DISPATCH) {
        const deliveryPromises = subscriptions.map(subscription => dispatchWebhook(subscription, payload)
            .catch(error => {
 ...
 */
const dispatchSignal = async (pool, corridorId, providerId, anomaly) => {
    if (!anomaly.detected || anomaly.zScore === null)
        return;
    const signalRepo = new repositories_1.SignalRepository(pool);
    ...
    const webhookRepo = new repositories_1.WebhookRepository(pool);
    const subscriptions = await webhookRepo.loadActiveSubscriptions(corridorId, providerId, anomaly.zScore);
    if (!subscriptions.length) {
    ...
    const payload = {
        type: config_2.SIGNAL_TYPES.ARBITRAGE_SIGNAL,
        corridor: corridorId,
        provider: providerId,
    ...
    const queued = notificationsQueueMode !== 'off'
        ? await enqueueNotification({
            signalType: config_2.SIGNAL_TYPES.ARBITRAGE_SIGNAL,
            corridorId,
            providerId,
            anomaly: {
                zScore: anomaly.zScore,
...
exports.dispatchSignal = dispatchSignal;
const dispatchQueuedSignal = async (pool, payload) => {
    if (!payload || typeof payload !== 'object')
        return;
    if (!payload.providerId || !payload.corridorId)
        return;
    ...
    const webhookRepo = new repositories_1.WebhookRepository(pool);
    const subscriptions = await webhookRepo.loadActiveSubscriptions(payload.corridorId, payload.providerId, payload.anomaly.zScore);
    if (!subscriptions.length) {
        logger.debug('no_webhook_subscriptions', {
            corridor_id: payload.corridorId,
            provider_id: payload.providerId,
...
exports.dispatchQueuedSignal = dispatchQueuedSignal;
/**
 ...
 */
const dispatchWebhook = async (subscription, payload) => {
    ...
    if (!currentSecret) {
        await (0, aws_services_1.recordNotificationMetric)('webhook', 'failed', 1);
    ...
    while (retryCount < config_2.WEBHOOK_CONFIG.MAX_RETRIES) {
        try {
            const timestamp = Date.now().toString();
            const payloadString = JSON.stringify(payload);
            const signature = (0, crypto_1.createHmac)('sha256', currentSecret)
                .update(`${timestamp}.${payloadString}`)
                .digest('hex');
            const previousSignature = previousSecret
                ? (0, crypto_1.createHmac)('sha256', previousSecret)
                    .update(`${timestamp}.${payloadString}`)
                    .digest('hex')
                : null;
            const response = await fetch(subscription.webhook_url, {
                ...
                body: payloadString,
                signal: AbortSignal.timeout(config_2.WEBHOOK_CONFIG.TIMEOUT_MS),
            });
            if (response.ok) {
                const durationMs = Date.now() - startTime;
                await (0, aws_services_1.recordNotificationMetric)('webhook', 'sent', 1);
                await (0, aws_services_1.recordNotificationDuration)('webhook', durationMs);
        ...
        retryCount += 1;
        if (retryCount < config_2.WEBHOOK_CONFIG.MAX_RETRIES) {
            const backoffMs = Math.min(Math.pow(2, retryCount) * config_2.WEBHOOK_CONFIG.BACKOFF_BASE_MS, config_2.WEBHOOK_CONFIG.MAX_BACKOFF_MS);
    ...
    logger.error('webhook_max_retries_exceeded', {
        ...
        status_code: null,
        success: false,
        retry_count: config_2.WEBHOOK_CONFIG.MAX_RETRIES,
        duration_ms: durationMs,
    });
};
/**
