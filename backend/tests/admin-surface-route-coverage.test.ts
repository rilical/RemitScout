...
vi.mock('../plane-a/src/services/newsletter-campaign', () => ({
  ...
  getActiveSubscriberCount: (...args: unknown[]) => mockGetActiveSubscriberCount(...args),
...
vi.mock('../plane-a/src/services/alert-evaluator', () => ({
  ...
  evaluateAlertsForFrequency: (...args: unknown[]) => mockEvaluateAlertsForFrequency(...args),
}))
...
vi.mock('../plane-a/src/repositories/implementations/module-registry-repository', () => ({
...
vi.mock('../plane-a/src/repositories/implementations/agent-actions-repository', () => ({
...
vi.mock('../plane-a/src/repositories/implementations/triangulated-index-repository', () => ({
  TriangulatedIndexRepository: class {
    getStressOverview = vi.fn()
  },
}))

vi.mock('../plane-a/src/repositories/implementations/correction-ledger-repository', () => ({
  CorrectionLedgerRepository: class {
    list = vi.fn()
  },
}))

vi.mock('../plane-a/src/repositories/implementations/data-quality-repository', () => ({
...
type RouteHandler = (request: RouteRequest, reply?: RouteReply) => Promise<unknown>
...
const getRouteOptions = (app: FastifyInstance, method: 'get' | 'post' | 'patch', url: string) => {
  const call = vi.mocked(app[method]).mock.calls.find((entry) => entry[0] === url)
  return call?.[1] as { preHandler?: Array<((...args: never[]) => unknown)> | ((...args: never[]) => unknown) }
}
...
describe('admin surface route coverage', () => {
  ...
  it('returns not_found for missing discovery scan detail', async () => {
    ...
    const app = makeApp()
    const { adminDiscoveryRoutes } = await import('../plane-a/src/routes/admin-discovery')
    await adminDiscoveryRoutes(app)

    const handler = getHandler(app, 'get', '/admin/discovery/scans/:scanId')
    await expect(handler({ params: { scanId: '42' } })).rejects.toBeInstanceOf(NotFoundError)
  })
  ...
  it('returns not_found for missing newsletter campaign detail', async () => {
    ...
    const app = makeApp()
    const { adminNewsletterRoutes } = await import('../plane-a/src/routes/admin-newsletter')
    await adminNewsletterRoutes(app)

    const handler = getHandler(app, 'get', '/admin/newsletter/campaigns/:id')
    await expect(handler({ params: { id: 'cmp_missing' } })).rejects.toBeInstanceOf(NotFoundError)
  })

  it('validates observer summary query params', async () => {
    const app = makeApp()
    const { observerSummaryRoutes } = await import('../plane-a/src/routes/ops/observer-summary')
    observerSummaryRoutes(app)

    const handler = getHandler(app, 'get', '/ops/observer/summary')
    await expect(handler({ query: { limit: '0' } }, { code: vi.fn().mockReturnThis() })).rejects.toBeInstanceOf(ValidationError)
  })

  it('validates stale API key query params', async () => {
    const app = makeApp()
    const { apiKeysAdminRoutes } = await import('../plane-a/src/routes/ops/api-keys-admin')
    apiKeysAdminRoutes(app)

    const handler = getHandler(app, 'get', '/ops/api-keys/stale')
    await expect(handler({ query: { staleDays: '0' } }, { code: vi.fn().mockReturnThis() })).rejects.toBeInstanceOf(ValidationError)
  })

  it('validates alert evaluation admin body requirements', async () => {
    const app = makeApp()
    const { alertEvaluationAdminRoutes } = await import('../plane-a/src/routes/ops/alert-evaluation-admin')
    alertEvaluationAdminRoutes(app)

    const handler = getHandler(app, 'post', '/ops/alerts/evaluate')
    await expect(handler({ body: {} }, { code: vi.fn().mockReturnThis() })).rejects.toBeInstanceOf(ValidationError)
  })
  ...
  it('blocks execute alert evaluation before side effects when audit preflight fails', async () => {
    ...
    const response = await handler({
      ...
      body: { frequency: 'daily', mode: 'execute', confirm: 'RUN', ignoreSchedule: true, limit: 5 },
    }, reply)
  ...
  it('validates db-admin request body', async () => {
    ...
    const handler = getHandler(app, 'post', '/ops/db/ensure-alert-notification-attempts')
    await expect(handler({ body: { mode: 'invalid' } }, { code: vi.fn().mockReturnThis() })).rejects.toBeInstanceOf(ValidationError)
  })
  ...
  it('rolls back db-admin ensure when audit logging fails', async () => {
    ...
    mockClientQuery
      ...
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce({ rows: [{ regclass: 'silver.alert_notification_attempt' }] })
      .mockResolvedValueOnce(undefined)
    mockLogAuditEvent.mockRejectedValueOnce(new Error('audit unavailable'))
    ...
    expect(mockClientQuery).toHaveBeenNthCalledWith(2, expect.stringContaining('CREATE TABLE IF NOT EXISTS silver.alert_notification_attempt'))
  ...
  it('returns a stable bad_request for invalid provider explain corridor ids', async () => {
    const app = makeApp()
    const { providersExplainRoutes } = await import('../plane-a/src/routes/ops/providers-explain')
  ...
  it('registers platform ops endpoints behind requireAdmin', async () => {
    const app = makeApp()
    const { platformOpsRoutes } = await import('../plane-a/src/routes/ops/platform')
    platformOpsRoutes(app)

    const options = getRouteOptions(app, 'get', '/ops/modules/health')
    const preHandlers = Array.isArray(options?.preHandler) ? options.preHandler : [options?.preHandler]
    const guardTags = preHandlers.map((handler) => (handler as { __guardTag?: string } | undefined)?.__guardTag)
  ...
  it('registers indices and B2B sweep status admin endpoints', async () => {
    const app = makeApp()
    const { indicesHealthRoutes } = await import('../plane-a/src/routes/ops/indices-health')
    ...
    expect(getRouteOptions(app, 'get', '/ops/indices/health')).toBeTruthy()
    expect(getRouteOptions(app, 'get', '/ops/b2b-sweep-status')).toBeTruthy()
  })
})
