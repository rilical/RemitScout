...
describe('API-key route matrix', () => {
  beforeEach(() => {
    ...
    mockQuery.mockImplementation(async (sql: string) => {
      ...
      if (text.includes('GROUP BY endpoint')) {
        return {
          rows: [{ endpoint: '/api/v1/indices/latest', requests: 9, avg_response_time_ms: 170 }],
          rowCount: 1,
        }
      }
      ...
      if (text.includes('COUNT(*)::int AS requests') && text.includes('FROM public.api_usage_log')) {
        return {
          rows: [
            {
              requests: 12,
  ...
  it('allows institutional keys on explicit indices routes', async () => {
  ...
  it('allows institutional keys on /usage and blocks retail keys there', async () => {
    const app = await buildApp()
    ...
    try {
      ...
      expect(institutionalRes.json()).toMatchObject({
        clientId: 'client-1',
        totals: { requests: 12 },
      })
      ...
      expect(retailRes.statusCode).toBe(403)
      expect(retailRes.json()).toMatchObject({
        error: 'forbidden',
  ...
  it('allows retail keys on /history/corridor and blocks institutional keys there', async () => {
    const app = await buildApp()
    ...
    try {
      ...
      expect(institutionalRes.statusCode).toBe(403)
      expect(institutionalRes.json()).toMatchObject({
        error: 'forbidden',
        code: 'api_key_route_not_allowed',
      })
  ...
  it('allows retail keys on /exports and blocks institutional keys there', async () => {
    const app = await buildApp()
  ...
  it('rejects API keys on public rates routes without explicit policy', async () => {
    const app = await buildApp()
