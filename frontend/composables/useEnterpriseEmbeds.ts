...
export function useEnterpriseEmbeds() {
  ...
  const copyPublishedValue = async (value: string, successMessage: string, emptyMessage: string) => {
  ...
  const fetchPublishedEmbeds = async () => {
    ...
    try {
      const response = await request<{ success: boolean; embeds: PublishedEmbedRecord[] }>('/me/published-embeds')
      publishedEmbeds.value = response.embeds ?? []
    } catch (error) {
      publishedEmbedsError.value = mapPlanStateFailureMessage(error, 'Unable to load published embeds.', {
  ...
  const revokePublishedEmbed = async (id: string) => {
    ...
    publishedEmbedsError.value = null
    try {
      const response = await request<{ success: boolean; embed: PublishedEmbedRecord }>(`/me/published-embeds/${id}/revoke`, {
        method: 'POST',
      })
      ...
      publishedEmbedsError.value = mapPlanStateFailureMessage(error, 'Unable to revoke published embed.', {
  ...
  watch([corridorId, amountBucket, methodProfile, days, theme], () => {
