...
type PlanStateMessageOverride =
  | string
  | ((failure: PlanStateFailure) => string)
...
const normalizeErrorData = (value: unknown): PlanStateErrorData => {
  ...
  const details = detailsValue && typeof detailsValue === 'object'
    ? {
        plan_failure: toTrimmedString((detailsValue as Record<string, unknown>).plan_failure) || undefined,
        required_plan: toTrimmedString((detailsValue as Record<string, unknown>).required_plan) || undefined,
        capability: toTrimmedString((detailsValue as Record<string, unknown>).capability) || undefined,
        lifecycle_state: toTrimmedString((detailsValue as Record<string, unknown>).lifecycle_state) || undefined,
        recovery_action: toTrimmedString((detailsValue as Record<string, unknown>).recovery_action) || undefined,
      }
    : null
...
export const extractPlanStateFailure = (error: unknown): PlanStateFailure => {
  ...
  return {
    ...
    message: data.message || toTrimmedString(normalized.message),
...
const defaultMessageForFailure = (failure: PlanStateFailure): string | null => {
  ...
  if (code === 'indices_export_enterprise_only') return 'TEER / RCI / RVI exports require an Enterprise plan.'
