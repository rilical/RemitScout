...
const { user, isPlus, signOut } = useAuth()
...
<template>
  <div class="relative user-menu">
    <button
      class="flex items-center gap-2 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
    ...
    >
      <div class="relative">
        ...
        <div
          v-else
          class="h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white text-sm font-semibold"
        >
          {{ user?.name?.charAt(0).toUpperCase() || 'U' }}
        </div>
        <span
          v-if="isPlus"
          class="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full bg-emerald-500 border-2 border-white"
          title="Plus member"
        />
      </div>
      <svg
