      ...
      <div class="relative w-full max-w-2xl rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800 to-slate-900 shadow-2xl">
        ...
        <div class="flex items-center justify-between border-b border-white/10 px-6 py-5">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h2 class="text-lg font-bold text-white">{{ corridorData.from }} → {{ corridorData.to }}</h2>
              <p class="text-sm text-slate-400">Live corridor data</p>
            </div>
          </div>
          <button
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-3 gap-4">
            <div class="rounded-xl bg-white/5 border border-white/5 p-4 text-center">
              <div class="text-xs text-slate-500 mb-1">Best rate</div>
              <div class="text-lg font-bold text-white">{{ corridorData.bestProvider }}</div>
            </div>
            <div class="rounded-xl bg-white/5 border border-white/5 p-4 text-center">
              <div class="text-xs text-slate-500 mb-1">Recipient gets</div>
              <div class="text-lg font-bold text-emerald-400">{{ corridorData.recipientGets }}</div>
            </div>
            <div class="rounded-xl bg-white/5 border border-white/5 p-4 text-center">
          ...
          <!-- Provider Comparison -->
          <div>
            <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Provider Comparison</h3>
            <div class="space-y-3">
              <div
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="font-semibold text-white">{{ provider.name }}</div>
                    <div class="text-xs text-slate-500">{{ provider.speed }}</div>
                  </div>
                </div>
                <div class="text-right">
                  <div class="font-bold text-white">{{ provider.recipientGets }}</div>
                  <div class="text-xs text-slate-400">Fee: {{ provider.fee }}</div>
                </div>
              </div>
            </div>
          </div>
