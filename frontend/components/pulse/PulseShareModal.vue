    ...
    >
      ...
      <div class="p-6">
        ...
        <template v-else-if="embedTab === 'iframe'">
          <p class="mb-4 text-neutral-400">
            Generate a static snapshot and paste this code into your website. No API key is required for viewers.
          </p>
          ...
          <div class="mb-4">
            <button
              class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-body-sm font-semibold text-white hover:bg-brand-700 transition-colors disabled:opacity-60"
              :disabled="embedGenerating"
              @click="generateEmbedSnapshot"
            >
              {{ embedGenerating ? 'Generating snapshot…' : 'Generate Static Snapshot' }}
            </button>
            <p class="mt-2 text-[11px] text-neutral-500">
              Static snapshots are generated by enterprise users and can be embedded publicly on blogs and media pages.
            </p>
          </div>

          <p
            v-if="embedError"
            class="mb-3 text-body-sm text-danger-600"
          >
            {{ embedError }}
          </p>
          ...
          <div class="relative">
            ...
            <button
              class="absolute top-2 right-2 flex items-center gap-1.5 rounded bg-neutral-700 px-2 py-1 text-body-sm text-white hover:bg-neutral-600 transition-colors disabled:opacity-60"
              :disabled="!embedCode"
              @click="copyEmbedCode"
            >
              <Icon
          ...
          <div class="mt-6">
            ...
            <div class="rounded-lg border border-neutral-700 bg-neutral-900 p-4">
              ...
              <div
                v-else
                class="flex h-[200px] items-center justify-center text-body-sm text-neutral-500"
              >
                Generate a snapshot to preview your embed.
              </div>
            </div>
          </div>
          ...
          <p class="mt-4 text-[11px] text-neutral-500">
            Embeds are generated by enterprise users as static snapshots and include a "Powered by Remit-Scout" backlink.
          </p>
        </template>
        ...
        <template v-else-if="embedTab === 'image'">
          ...
          <div class="mt-6 rounded-lg border border-neutral-700 bg-neutral-900 p-4">
            ...
            <ul class="space-y-1 text-[11px] text-neutral-400">
              ...
              <li>Use the embed code above to publish a static chart snapshot on your site</li>
            </ul>
          </div>
        </template>
      </div>
...
import { createPulseEmbedSnapshot } from '~/lib/pulseApi'
import { Icon } from '~/ui'
...
const embedSnapshotId = ref<string | null>(null)
...
const embedUrl = computed(() => {
  if (!embedSnapshotId.value) return ''
  const params = new URLSearchParams()
  params.set('snapshot_id', embedSnapshotId.value)
...
async function generateEmbedSnapshot() {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to generate embed snapshot.'
    embedError.value = message
  }
  finally {
...
async function copyEmbedCode() {
  if (!embedUrl.value) {
    embedError.value = 'Generate a snapshot before copying embed code.'
    return
  }
  try {
