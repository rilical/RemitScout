    ...
    <div class="mx-auto flex max-w-6xl flex-col gap-6">
      <header class="rounded-2xl bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 class="text-2xl font-semibold text-slate-900">Audit Log Console</h1>
            <p class="text-sm text-slate-500">Security, compliance, and user action trails.</p>
          </div>
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-4">
          <label class="text-xs text-slate-500">
            Actor ID
            <input v-model="filters.actor_id" type="text" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          </label>
          <label class="text-xs text-slate-500">
            Action
            <input v-model="filters.action" type="text" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
          </label>
          <label class="text-xs text-slate-500">
            Category
            <select v-model="filters.category" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
          ...
          <label class="text-xs text-slate-500">
            Severity
            <select v-model="filters.severity" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
              <option value="">All</option>
      ...
      <section class="rounded-2xl bg-white p-6 shadow-sm">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold text-slate-900">Audit Events</h2>
        ...
        <div class="mt-4 overflow-auto">
          <table class="min-w-full text-sm">
            ...
            <tbody>
              <tr v-for="log in logs" :key="log.event_id" class="border-t border-slate-100">
                <td class="py-2 text-left text-slate-600">{{ formatDate(log.created_at) }}</td>
                <td class="py-2 text-left text-slate-700">{{ log.action }}</td>
                <td class="py-2 text-left text-slate-600">{{ log.actor_id }}</td>
                <td class="py-2 text-left text-slate-600">
                  {{ log.entity_type }}{{ log.entity_id ? `:${log.entity_id}` : '' }}
                </td>
                <td class="py-2 text-left text-slate-600">{{ log.category }}</td>
                <td class="py-2 text-left text-slate-600">{{ log.severity || 'info' }}</td>
              </tr>
              <tr v-if="logs.length === 0">
                <td colspan="6" class="py-3 text-center text-xs text-slate-400">No audit events found.</td>
              </tr>
            </tbody>
          </table>
        </div>
