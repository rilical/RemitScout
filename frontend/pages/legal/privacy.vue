<template>
  <div class="min-h-screen bg-gray-50">
    <div class="container mx-auto px-4 py-8">
      <Breadcrumbs :items="breadcrumbItems" />

      <div class="mx-auto max-w-4xl">
        <div class="rounded-lg bg-white p-8 shadow-md">
          <div class="mb-8">
            <h1 class="mb-4 text-4xl font-bold text-gray-900">Privacy Policy</h1>
            <p class="text-gray-600">Last updated: November 22, 2025</p>
          </div>

          <div class="prose prose-lg max-w-none">
            <p class="mb-6 text-lg leading-relaxed">
              <strong class="text-blue-600">Remit-Scout</strong> ("we," "our," "us") is committed to protecting your privacy. 
              This Privacy Policy describes how we collect, use, and safeguard information when you use our money transfer 
              comparison tools.
            </p>

            <div class="my-8 rounded-lg border-l-4 border-blue-500 bg-blue-50 p-6">
              <h3 class="mb-2 text-lg font-semibold text-blue-900">No Account Required</h3>
              <p class="text-blue-800 leading-relaxed">
                We do <strong>not</strong> create user accounts, and we do <strong>not</strong> collect personally identifiable 
                information such as names, passwords, or payment details. Our service is designed to be simple and privacy-focused: 
                you can compare money transfer providers without signing up or logging in. Learn more about our 
                <NuxtLink to="/about" class="font-semibold underline hover:text-blue-900">mission and approach</NuxtLink>.
              </p>
            </div>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">1. Information We Collect</h2>

            <h3 class="mb-3 text-xl font-semibold text-gray-900">A. Information You Provide to Us</h3>
            <p class="mb-4 leading-relaxed">
              Because <strong class="text-blue-600">Remit-Scout</strong> does not offer account creation, we only collect 
              information you voluntarily share when you contact us:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Email address</strong> (if you contact us)</li>
              <li><strong class="text-blue-600">Messages or inquiries</strong> you send to us</li>
              <li><strong class="text-blue-600">Feedback or error reports</strong> you submit</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              This information is used only to respond to you and is not used for marketing or shared with third parties.
            </p>

            <h3 class="mb-3 text-xl font-semibold text-gray-900">B. Information Collected Automatically</h3>
            <p class="mb-4 leading-relaxed">
              When you use our comparison tool, we automatically collect <strong class="text-blue-600">non-personal, 
              usage-based data</strong>, including:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Search Inputs:</strong> Transfer amount, sending country, receiving country, 
              and corridor used to generate comparison results</li>
              <li><strong class="text-blue-600">Provider Selection:</strong> Which providers you clicked, visited, or compared</li>
              <li><strong class="text-blue-600">Device & Technical Information:</strong> IP address (used only to determine 
              approximate location/country), browser type, device type, operating system</li>
              <li><strong class="text-blue-600">Usage Data:</strong> Pages visited, time spent, buttons clicked, referral URLs</li>
              <li><strong class="text-blue-600">Location Data (approximate):</strong> We infer your <strong>country/region only</strong>, 
              based on IP, to show correct comparison results. We do <em>not</em> track precise location</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              Tracked data is used for analytics and to improve our comparison service. We do <strong>not</strong> profile you, 
              build personal datasets, or store your comparison history in a way that identifies you.
            </p>

            <h3 class="mb-3 text-xl font-semibold text-gray-900">C. Cookies & Tracking Technologies</h3>
            <p class="mb-4 leading-relaxed">
              We use cookies and similar technologies to:
            </p>
            <ul class="mb-4 list-inside list-disc space-y-2 leading-relaxed">
              <li>Operate essential website features</li>
              <li>Understand how users interact with the comparison tool</li>
              <li>Improve performance and user experience</li>
              <li>Measure provider referral performance</li>
              <li>Deliver relevant marketing (if applicable)</li>
            </ul>
            <p class="mb-4 leading-relaxed">
              Types of cookies used:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Essential cookies:</strong> Required for the website to function properly</li>
              <li><strong class="text-blue-600">Analytics cookies:</strong> Help us understand usage patterns (e.g., Google Analytics)</li>
              <li><strong class="text-blue-600">Marketing/retargeting cookies:</strong> Used for advertising purposes (e.g., Meta, TikTok)</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              You may control or delete cookies through your browser settings. Disabling certain cookies may limit your ability 
              to use some features of our website.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">2. How We Use Your Information</h2>
            <p class="mb-4 leading-relaxed">
              We use the data we collect to:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Generate accurate money transfer comparisons</strong> based on your search inputs</li>
              <li><strong class="text-blue-600">Track provider clicks</strong> for referral attribution and reporting</li>
              <li><strong class="text-blue-600">Improve algorithms, accuracy, and speed</strong> of our comparison tool</li>
              <li><strong class="text-blue-600">Detect errors and prevent misuse</strong> of our platform</li>
              <li><strong class="text-blue-600">Understand trends and platform usage</strong> to enhance user experience</li>
              <li><strong class="text-blue-600">Measure which corridors are most used</strong> to prioritize improvements</li>
              <li><strong class="text-blue-600">Comply with legal obligations</strong> and respond to legal requests</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              We do <strong>not</strong> profile you, build personal datasets, or store your comparison history in a way that 
              identifies you as an individual.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">3. Information Sharing</h2>
            <p class="mb-4 leading-relaxed">
              We do <strong>not</strong> sell, rent, or trade your personal information.
            </p>
            <p class="mb-4 leading-relaxed">
              We may share non-personal usage data with:
            </p>

            <h3 class="mb-3 text-xl font-semibold text-gray-900">A. Service Providers</h3>
            <p class="mb-4 leading-relaxed">
              Trusted partners that help us operate the website, including:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li>Analytics tools (e.g., Google Analytics)</li>
              <li>Cloud hosting services</li>
              <li>Anti-fraud/security tools</li>
              <li>Email support software</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              These providers can only use data for our purposes and must protect it in accordance with our agreements.
            </p>

            <h3 class="mb-3 text-xl font-semibold text-gray-900">B. Money Transfer Providers</h3>
            <p class="mb-4 leading-relaxed">
              When you click a provider link (e.g., Wise, Remitly, WorldRemit), you are redirected to their website.
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li>We <strong>do not</strong> send your personal data to providers</li>
              <li>Providers may independently track referrals or performance</li>
              <li>Their privacy policies apply once you leave <strong class="text-blue-600">Remit-Scout</strong></li>
            </ul>

            <h3 class="mb-3 text-xl font-semibold text-gray-900">C. Legal Compliance & Safety</h3>
            <p class="mb-6 leading-relaxed">
              We may disclose information if required to:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li>Comply with applicable laws, regulations, or legal processes</li>
              <li>Respond to valid legal requests from government authorities</li>
              <li>Protect our rights, property, or security, or that of our users</li>
              <li>Prevent fraud, abuse, or illegal activity</li>
            </ul>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">4. Data Retention</h2>
            <p class="mb-4 leading-relaxed">
              Since we do not create accounts, we only retain:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Analytics data:</strong> Aggregated and anonymized usage statistics</li>
              <li><strong class="text-blue-600">Click-through/referral data:</strong> Needed for provider reporting and attribution</li>
              <li><strong class="text-blue-600">Communication data:</strong> Any messages or inquiries you send to us</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              We keep this data only for as long as necessary for operational, reporting, or legal purposes, then delete or 
              anonymize it. Most usage data is retained for up to 2 years, after which it is automatically deleted or anonymized.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">5. Security</h2>
            <p class="mb-4 leading-relaxed">
              We use standard security measures to protect your information, including:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Encryption of data in transit</strong> using SSL/TLS protocols</li>
              <li><strong class="text-blue-600">Secure hosting environments</strong> with regular security updates</li>
              <li><strong class="text-blue-600">Access controls</strong> to limit who can view or modify data</li>
              <li><strong class="text-blue-600">Monitoring and threat detection</strong> to identify and respond to security issues</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              While we strive to protect your data, no system is fully secure. We cannot guarantee absolute security, but we 
              implement industry-standard practices to safeguard your information.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">6. Your Rights</h2>
            <p class="mb-4 leading-relaxed">
              Depending on your location, you may have rights to:
            </p>
            <ul class="mb-6 list-inside list-disc space-y-2 leading-relaxed">
              <li><strong class="text-blue-600">Request deletion</strong> of communication data you sent to us</li>
              <li><strong class="text-blue-600">Opt out</strong> of analytics or marketing tracking</li>
              <li><strong class="text-blue-600">Request a copy</strong> of any personal information we hold (typically none, 
              since we don't create accounts)</li>
              <li><strong class="text-blue-600">Restrict or object</strong> to certain data uses</li>
              <li><strong class="text-blue-600">Data portability</strong> (where applicable)</li>
            </ul>
            <p class="mb-6 leading-relaxed">
              To exercise these rights, please <NuxtLink to="/contact" class="text-blue-600 hover:text-blue-700 underline font-semibold">contact us</NuxtLink>. 
              We will respond to your request within a reasonable timeframe and in accordance with applicable laws.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">7. Children's Privacy</h2>
            <p class="mb-6 leading-relaxed">
              <strong class="text-blue-600">Remit-Scout</strong> is not intended for users under 18. We do not knowingly collect 
              any information from children. If you believe we have inadvertently collected information from a child, please 
              <NuxtLink to="/contact" class="text-blue-600 hover:text-blue-700 underline font-semibold">contact us</NuxtLink> 
              immediately, and we will take steps to delete such information.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">8. International Data Transfers</h2>
            <p class="mb-6 leading-relaxed">
              Your information may be transferred to and processed in countries other than your country of residence. These 
              countries may have data protection laws that differ from those in your country. We take appropriate safeguards to 
              ensure your information receives adequate protection in accordance with this Privacy Policy.
            </p>

            <h2 class="mb-4 text-2xl font-bold text-gray-900">9. Changes to This Privacy Policy</h2>
            <p class="mb-6 leading-relaxed">
              We may update this Privacy Policy periodically to reflect changes in our practices, technology, legal requirements, 
              or other factors. The updated version will have a new "Last updated" date. We encourage you to review this policy 
              from time to time to stay informed about how we protect your information.
            </p>

            <!-- CTA Section -->
            <div class="my-12 rounded-xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-white p-8">
              <h3 class="mb-4 text-2xl font-bold text-gray-900">Learn More About Remit-Scout</h3>
              <p class="mb-6 text-gray-700 leading-relaxed">
                Transparency and trust are at the heart of everything we do. Learn more about our mission, how we operate, 
                and our commitment to helping you make informed financial decisions.
              </p>
              <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <NuxtLink 
                  to="/about" 
                  class="flex items-center gap-3 rounded-lg border border-blue-200 bg-white p-4 transition-all hover:border-blue-300 hover:bg-blue-50 hover:shadow-md"
                >
                  <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                  <div>
                    <div class="font-semibold text-gray-900">About Us</div>
                    <div class="text-sm text-gray-600">Our story and mission</div>
                  </div>
                </NuxtLink>
                <NuxtLink 
                  to="/methodology" 
                  class="flex items-center gap-3 rounded-lg border border-blue-200 bg-white p-4 transition-all hover:border-blue-300 hover:bg-blue-50 hover:shadow-md"
                >
                  <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                  </svg>
                  <div>
                    <div class="font-semibold text-gray-900">Methodology</div>
                    <div class="text-sm text-gray-600">How we compare providers</div>
                  </div>
                </NuxtLink>
                <NuxtLink 
                  to="/legal/disclosure" 
                  class="flex items-center gap-3 rounded-lg border border-blue-200 bg-white p-4 transition-all hover:border-blue-300 hover:bg-blue-50 hover:shadow-md"
                >
                  <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                  <div>
                    <div class="font-semibold text-gray-900">Disclosure Policy</div>
                    <div class="text-sm text-gray-600">Our transparency commitment</div>
                  </div>
                </NuxtLink>
                <NuxtLink 
                  to="/faq" 
                  class="flex items-center gap-3 rounded-lg border border-blue-200 bg-white p-4 transition-all hover:border-blue-300 hover:bg-blue-50 hover:shadow-md"
                >
                  <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <div>
                    <div class="font-semibold text-gray-900">FAQ</div>
                    <div class="text-sm text-gray-600">Common questions</div>
                  </div>
                </NuxtLink>
                <NuxtLink 
                  to="/contact" 
                  class="flex items-center gap-3 rounded-lg border border-blue-200 bg-white p-4 transition-all hover:border-blue-300 hover:bg-blue-50 hover:shadow-md"
                >
                  <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  <div>
                    <div class="font-semibold text-gray-900">Contact Us</div>
                    <div class="text-sm text-gray-600">Get in touch</div>
                  </div>
                </NuxtLink>
                <NuxtLink 
                  to="/legal/terms" 
                  class="flex items-center gap-3 rounded-lg border border-blue-200 bg-white p-4 transition-all hover:border-blue-300 hover:bg-blue-50 hover:shadow-md"
                >
                  <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  <div>
                    <div class="font-semibold text-gray-900">Terms of Service</div>
                    <div class="text-sm text-gray-600">Legal agreement</div>
                  </div>
                </NuxtLink>
              </div>
            </div>

            <div class="rounded-lg border border-blue-200 bg-blue-50 p-6">
              <h3 class="mb-2 text-lg font-semibold text-blue-900">Questions About Our Privacy Policy?</h3>
              <p class="mb-4 text-blue-800 leading-relaxed">
                If you have questions about how we collect, use, or protect your information, or if you wish to exercise 
                your privacy rights, we're here to help. Visit our 
                <NuxtLink to="/faq" class="font-semibold underline hover:text-blue-900">FAQ page</NuxtLink> 
                or <NuxtLink to="/contact" class="font-semibold underline hover:text-blue-900">contact us directly</NuxtLink>.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import Breadcrumbs from '~/components/shared/Breadcrumbs.vue'

// Meta
useHead({
  title: 'Privacy Policy | How We Protect Your Data | Remit-Scout',
  meta: [
    {
      name: 'description',
      content:
        "Read Remit-Scout's Privacy Policy. We don't require accounts or collect personal information. Learn how we protect your privacy while providing money transfer comparisons.",
    },
    {
      name: 'keywords',
      content: 'privacy policy, data protection, no account required, cookies, GDPR, Remit-Scout',
    },
  ],
});

// Breadcrumbs
const breadcrumbItems = [
  { name: 'Home', path: '/' },
  { name: 'Legal', path: '/legal' },
  { name: 'Privacy Policy', path: '/legal/privacy' },
];
</script>
