<template>
  <div class="min-h-screen bg-slate-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8">
        <div class="flex items-center gap-3 mb-2">
          <NuxtLink to="/dashboard" class="text-slate-500 hover:text-slate-700">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
          </NuxtLink>
          <h1 class="text-2xl font-bold text-slate-900">Data Exports</h1>
      ...
      <div v-if="!isEnterprise" class="bg-white rounded-2xl border border-slate-200 p-8 text-center">
        <div class="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg class="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
        </div>
        <h2 class="text-xl font-bold text-slate-900 mb-2">Enterprise Feature</h2>
      ...
      <div v-else class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="bg-white rounded-2xl border border-slate-200 p-6">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="font-semibold text-slate-900">API Status</h3>
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-6">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="font-semibold text-slate-900">Data Freshness</h3>
                <p class="text-sm text-slate-500">By corridor tier</p>
              </div>
            </div>
            <div class="text-sm text-slate-600">
              <div class="flex justify-between py-1">
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-6">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="font-semibold text-slate-900">Indices Available</h3>
                <p class="text-sm text-slate-500">Gold export</p>
              </div>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div class="p-6 border-b border-slate-200">
            <div class="flex items-center justify-between">
              <div>
                <h2 class="text-lg font-semibold text-slate-900">Create New Export</h2>
                <p class="text-sm text-slate-500">Export corridor data in CSV or PDF format</p>
              </div>
            </div>
          </div>

          <div class="p-6">
            <form @submit.prevent="createExport" class="space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  >
                    ...
                    <option value="watchlist">Watchlist Data</option>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div v-if="exportsLoading" class="p-8 text-center text-slate-500">
          ...
          <div v-else-if="exports.length === 0" class="p-8 text-center text-slate-500">
          ...
          <table v-else class="w-full">
            <thead class="bg-slate-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Format</th>
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Expires</th>
                <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-200">
              <tr v-for="exp in exports" :key="exp.id" class="hover:bg-slate-50">
                <td class="px-6 py-4 text-sm text-slate-900 capitalize">{{ formatJobType(exp.jobType) }}</td>
                <td class="px-6 py-4 text-sm text-slate-600 uppercase">{{ getFormat(exp.jobType) }}</td>
                <td class="px-6 py-4">
                  <span
                    :class="[
                ...
                <td class="px-6 py-4 text-sm text-slate-600">{{ formatDate(exp.createdAt) }}</td>
                <td class="px-6 py-4 text-sm text-slate-600">{{ exp.expiresAt ? formatDate(exp.expiresAt) : '-' }}</td>
                <td class="px-6 py-4 text-right">
                  ...
                  >
                    <svg v-if="downloadingId === exp.id" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
                    <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <span v-else-if="exp.status === 'queued' || exp.status === 'running'" class="text-sm text-slate-400">
                    Processing...
                  </span>
                  <span v-else-if="exp.status === 'failed'" class="text-sm text-red-500">
                    Failed
                  </span>
                </td>
              </tr>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div class="p-6 border-b border-slate-200">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-sm text-slate-500">Manage your API keys for programmatic access</p>
              </div>
              <button
          ...
          <div v-if="apiKeys.length === 0" class="p-8 text-center text-slate-500">
            <svg class="w-12 h-12 mx-auto mb-3 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <table v-else class="w-full">
            <thead class="bg-slate-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Key Prefix</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Scopes</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Created</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Last Used</th>
                <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-200">
              <tr v-for="key in apiKeys" :key="key.key_id" class="hover:bg-slate-50">
                <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ key.name || 'Unnamed Key' }}</td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <div class="flex flex-wrap gap-1">
                    ...
                    <span v-if="key.scopes.length > 3" class="text-xs text-slate-400">
                      +{{ key.scopes.length - 3 }} more
                    </span>
                  </div>
                </td>
                <td class="px-6 py-4 text-sm text-slate-600">{{ formatDate(key.created_at) }}</td>
                <td class="px-6 py-4 text-sm text-slate-600">{{ key.last_used_at ? formatDate(key.last_used_at) : 'Never' }}</td>
                <td class="px-6 py-4 text-right">
                  <button
        ...
        <div class="bg-blue-50 rounded-2xl border border-blue-200 p-6">
          ...
          <div class="mt-4 flex gap-4">
            <NuxtLink to="/institutions/api" class="text-sm font-medium text-blue-600 hover:text-blue-700">
    ...
    <Teleport to="body">
      <div v-if="showCreateKeyModal" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/50" @click="showCreateKeyModal = false" />
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
          <h3 class="text-lg font-semibold text-slate-900 mb-4">Create API Key</h3>
    ...
    <Teleport to="body">
      <div v-if="newKeyToken" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6">
          <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-lg font-semibold text-slate-900">API Key Created</h3>
          </div>
...
const isEnterprise = computed(() => {
  return user.value?.user_metadata?.plan === 'enterprise'
})
...
const loadExports = async () => {
  ...
  try {
    const { data } = await useFetch('/api/v1/exports', {
      headers: {
        Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
    })
    if (data.value?.jobs) {
      exports.value = data.value.jobs
    }
  } catch (error) {
...
const loadApiKeys = async () => {
  if (!isEnterprise.value) return
  try {
    const { data } = await useFetch('/api/v1/me/api-keys', {
      headers: {
        Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
    })
...
const createExport = async () => {
  ...
  try {
    const { data, error } = await useFetch('/api/v1/exports', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
    ...
    if (data.value?.job) {
      exports.value.unshift(data.value.job)
    }
  } catch (error) {
...
const downloadExport = async (id: string) => {
  ...
  try {
    const { data, error } = await useFetch(`/api/v1/exports/${id}/download`, {
      headers: {
        Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
    })
...
const createApiKey = async () => {
  ...
  try {
    const { data, error } = await useFetch('/api/v1/me/api-keys', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
    ...
    if (error.value) throw error.value
    if (data.value?.token) {
      newKeyToken.value = data.value.token
...
const revokeKey = async (keyId: string) => {
  ...
  try {
    await useFetch(`/api/v1/me/api-keys/${keyId}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
