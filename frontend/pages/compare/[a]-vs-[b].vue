<template>
  <div class="min-h-screen bg-gray-50">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
          <div class="flex items-center">
            ...
            <span class="font-medium text-blue-800"
              >Comparison based on current rates and user reviews</span
            >
          </div>
        </div>
      </div>

      <ComparisonTable :providers="[providerA, providerB]" :comparison="true" />

      <div class="mb-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
        <div class="rounded-lg bg-white p-6 shadow-md">
          <h2 class="mb-4 text-2xl font-bold text-gray-900">{{ providerA?.name }} Overview</h2>
          <div class="space-y-4">
        ...
        <div class="rounded-lg bg-white p-6 shadow-md">
          <h2 class="mb-4 text-2xl font-bold text-gray-900">{{ providerB?.name }} Overview</h2>
          <div class="space-y-4">
            <div class="flex items-center justify-between border-b py-2">
              <span class="text-gray-600">Rating</span>
              <div class="flex items-center">
      ...
      <div class="rounded-lg bg-white p-6 shadow-md">
        <h2 class="mb-4 text-2xl font-bold text-gray-900">Which Should You Choose?</h2>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            ...
            <ul class="list-inside list-disc space-y-1 text-gray-600">
              <li v-for="reason in providerA?.bestFor" :key="reason">
                {{ reason }}
              </li>
            </ul>
          </div>
          <div>
            ...
            <ul class="list-inside list-disc space-y-1 text-gray-600">
              <li v-for="reason in providerB?.bestFor" :key="reason">
                {{ reason }}
              </li>
            </ul>
          </div>
