        ...
        <div class="grid gap-8 lg:grid-cols-2 lg:items-center">
          ...
          <div>
            ...
            <div class="mb-4 grid gap-3 sm:grid-cols-2">
              <div class="flex h-full flex-col rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
              <button
                type="button"
                class="text-body-sm inline-flex items-center gap-2 rounded-xl border border-white/10 bg-surface/10 px-4 py-2.5 font-semibold text-white backdrop-blur-sm hover:bg-surface/20 disabled:cursor-not-allowed disabled:opacity-60 motion-safe:transition-all"
        ...
        <template v-if="!showRefreshGate">
          ...
          >
            <div class="mx-auto max-w-md">
              <template v-if="displayAmount < suggestedMinAmount">
                <div
                  class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-brand-100"
                >
                  <svg
                ...
                <p class="mt-2 text-sm leading-relaxed text-brand-700/80">
                  ...
                  <span class="font-semibold text-brand-800">{{
                    formatCurrency(suggestedMinAmount, fromCurrencyCode)
                  }}</span>
              ...
              <template v-else>
                ...
                <p class="mt-2 text-sm leading-relaxed text-brand-700/80">
                  None of the providers we track currently cover this corridor. Try a different
                  destination or currency pair.
                </p>
              </template>
            </div>
          </div>
          ...
          >
            <template
              ...
              :key="row.providerId || row.provider"
            >
              <ProviderComparisonCard
                :id="`provider-${row.provider.toLowerCase().replace(/\s+/g, '-')}`"
                :row="row"
                :is-top-ranked="index === 0 && !row.isStale"
                :highlight-label="index === 0 && !row.isStale ? topProviderLabel : ''"
                :from-currency-code="fromCurrencyCode"
                :to-currency-code="toCurrencyCode"
                :mid-market-rate="midMarketRate"
                @open-score="openScoreModal(row)"
                @outbound="handleProviderOutbound(row)"
              />
          ...
          <div class="space-y-6">
            <div class="rounded-xl border-2 border-brand-600 bg-brand-600 p-5">
              ...
              <div class="space-y-3">
                <button
                  type="button"
                  class="text-body flex w-full items-center justify-center gap-2 rounded-lg bg-surface px-4 py-3 font-bold text-brand-700 shadow-md hover:bg-brand-50 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-60 motion-safe:transition-colors"
                  :aria-pressed="isCorridorSaved"
                  :disabled="!actionControlsReady"
                ...
                <button
                  type="button"
                  class="text-body flex w-full items-center justify-center gap-2 rounded-lg border-2 border-white bg-transparent px-4 py-3 font-bold text-white shadow-md hover:bg-surface hover:text-brand-700 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-60 motion-safe:transition-colors"
                  :aria-pressed="hasCorridorAlerts"
                  :disabled="!actionControlsReady"
...
const apiRows = computed<TableRow[]>(() => {
  ...
  return ratedQuotes.value.map((quote, index) => {
    ...
    return {
      ...
      recipientGetsAmount: quote.recipientGets,
...
.loading-bar-animate {
  width: 38%;
  animation: loading-bar 1.45s cubic-bezier(0.22, 1, 0.36, 1) infinite;
