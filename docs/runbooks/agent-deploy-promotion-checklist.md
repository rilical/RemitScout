...
- [ ] Run staging readiness gate (must be green):
  ...
  - The readiness workflow is responsible for building the exact-SHA migration container, running it inside the staging ECS/VPC runtime, and proving post-migration tip sync before smoke evidence is accepted.
