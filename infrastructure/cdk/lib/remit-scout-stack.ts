...
const toEnvBoolString = (value: boolean): string => (value ? 'true' : 'false')
...
const toInterfaceEndpointMode = (
  value: unknown,
): InterfaceEndpointMode | undefined => {
  if (typeof value !== 'string') return undefined
  const normalized = value.trim().toLowerCase()
  if (
    normalized === 'all' ||
...
const resolveArchitecture = (
  value?: string,
): { cpu: CpuArchitecture; lambda: Architecture } => {
...
export class RemitScoutStack extends Stack {
  constructor(scope: Construct, id: string, props: RemitScoutStackProps) {
    ...
    const ownerTag =
      this.node.tryGetContext('tagOwner') ?? process.env.TAG_OWNER
    if (ownerTag) {
    ...
    if (!sharedSecretArn) {
      throw new Error(
        'sharedSecretArn context or SHARED_SECRET_ARN env var required',
      )
    }
    ...
    if (envName !== 'dev') {
      if (sesIdentityArns.length === 0) {
        throw new Error(
          'sesIdentityArns context or SES_IDENTITY_ARNS env var required for staging/prod',
        )
      }
      if (snsTopicArns.length === 0) {
        throw new Error(
          'snsTopicArns context or SNS_TOPIC_ARNS env var required for staging/prod',
        )
      }
    }
    const enableDbProxy =
      toOptionalBool(
        this.node.tryGetContext('enableDbProxy') ?? process.env.ENABLE_DB_PROXY,
      ) ?? envName !== 'dev'
    const prodDatabaseInstances =
      envName === 'prod'
        ? toOptionalNumber(
    ...
    if (
      envName === 'prod' &&
      prodDatabaseInstances !== undefined &&
      prodDatabaseInstances < 1
    ) {
      throw new Error('prodDatabaseInstances must be >= 1 in prod.')
    }
    const prodRedisNodeType =
      envName === 'prod'
        ? toOptionalString(
        ...
        : undefined
    const prodRedisReplicasPerNodeGroup =
      envName === 'prod'
        ? toOptionalNumber(
    ...
    const prodRedisAutomaticFailoverEnabled =
      envName === 'prod'
        ? toOptionalBool(
    ...
    const prodRedisMultiAzEnabled =
      envName === 'prod'
        ...
        : undefined
    const resolvedProdRedisNodeType =
      envName === 'prod' ? (prodRedisNodeType ?? 'cache.t4g.small') : undefined
    const resolvedProdRedisReplicasPerNodeGroup =
      envName === 'prod' ? (prodRedisReplicasPerNodeGroup ?? 1) : undefined
    const resolvedProdRedisAutomaticFailover =
      envName === 'prod'
        ? (prodRedisAutomaticFailoverEnabled ??
          (resolvedProdRedisReplicasPerNodeGroup ?? 0) > 0)
        : undefined
    const resolvedProdRedisMultiAz =
      envName === 'prod'
        ? (prodRedisMultiAzEnabled ??
          (resolvedProdRedisReplicasPerNodeGroup ?? 0) > 0)
        : undefined
    if (
      envName === 'prod' &&
    ...
    if (
      envName === 'prod' &&
      resolvedProdRedisMultiAz &&
      !resolvedProdRedisAutomaticFailover
    ) {
      throw new Error(
    ...
    const redisAuthMode =
      toRedisAuthMode(
        this.node.tryGetContext('redisAuthMode') ?? process.env.REDIS_AUTH_MODE,
      ) ?? (envName === 'dev' ? 'legacy' : 'required')
    if (
      (envName === 'staging' || envName === 'prod') &&
      redisAuthMode !== 'required'
    ) {
      throw new Error(
    ...
    const enableBackup =
      toOptionalBool(
        this.node.tryGetContext('enableBackup') ?? process.env.ENABLE_BACKUP,
      ) ?? envName !== 'dev'
    ...
    const pinpointEnabled =
      toOptionalBool(
        cdkContext.pinpointEnabled ?? process.env.PINPOINT_ENABLED,
      ) ?? envName !== 'dev'
    ...
    const minimalInfra =
      toOptionalBool(
        this.node.tryGetContext('minimalInfra') ?? process.env.MINIMAL_INFRA,
      ) ?? false
    ...
    if (
      enableCostGuardrails &&
      envName !== 'dev' &&
      costAlertEmailsExplicitlyEmpty
    ) {
      throw new Error(
    ...
    if (
      envName !== 'dev' &&
      typeof natGateways === 'number' &&
      natGateways < 1
    ) {
      throw new Error(
        'stagingNatGateways/prodNatGateways must be >= 1 in non-dev environments.',
      )
    }
    const stagingInterfaceEndpointsMode =
      envName === 'staging'
    ...
    const stagingInterfaceEndpointAllowlist =
      envName === 'staging'
        ? toList(
    ...
    const prodInterfaceEndpointsMode =
      envName === 'prod'
        ...
        : undefined
    const interfaceEndpointMode: InterfaceEndpointMode =
      envName === 'staging'
    ...
    const networking = createNetworking(this, {
      ...
      interfaceEndpointAllowlist:
        envName === 'staging'
          ? resolvedStagingInterfaceEndpointAllowlist
          : undefined,
    })
    ...
    let redisSecretArn =
      this.node.tryGetContext('redisSecretArn') ?? process.env.REDIS_SECRET_ARN
    const redisSecretJsonKey =
      this.node.tryGetContext('redisSecretJsonKey') ??
      process.env.REDIS_SECRET_JSON_KEY
    const redisSsmName =
      this.node.tryGetContext('redisSsmName') ?? process.env.REDIS_SSM_NAME
    ...
    const stripeSecretArn =
      ...
      sharedSecretArn
    const stripeSsmName =
      this.node.tryGetContext('stripeSsmName') ?? process.env.STRIPE_SSM_NAME
    ...
    const oandaSecretArn =
      this.node.tryGetContext('oandaSecretArn') ?? process.env.OANDA_SECRET_ARN
    const oandaSsmName =
      this.node.tryGetContext('oandaSsmName') ?? process.env.OANDA_SSM_NAME
    ...
    if (
      envName !== 'dev' &&
      planeADesiredCount !== undefined &&
      planeADesiredCount < 1
    ) {
      throw new Error('planeADesiredCount must be >= 1 outside dev.')
    }
    const planeBIngestDesiredCount =
      toOptionalNumber(
      ...
      (envName === 'prod'
        ...
        : envName === 'staging'
          ...
          : envName === 'dev'
            ? 1
            : undefined)
    const fxRateRefreshDesiredCount =
      toOptionalNumber(
    ...
    if (snowflakePartnerAccountIds.length > 0) {
      if (!snowflakePartnerExternalId) {
        Annotations.of(this).addWarning(
          'snowflakePartnerAccountIds configured without snowflakePartnerExternalId; skipping partner role.',
        )
      } else {
        const principals = snowflakePartnerAccountIds.map(
        ...
        const role = new Role(this, 'SnowflakePartnerRole', {
          roleName:
            snowflakePartnerRoleName ||
            `remit-scout-${envName}-snowflake-partner`,
          assumedBy,
        })
        ...
        role.addToPolicy(
          new PolicyStatement({
            ...
            resources: allowedPrefixes.map((prefix) =>
              storage.exportsBucket.arnForObjects(`${prefix}*`),
            ),
          }),
        )
        role.addToPolicy(
        ...
        storage.exportsBucket.addToResourcePolicy(
          new PolicyStatement({
            principals: [role],
            actions: ['s3:GetObject', 's3:GetObjectVersion'],
            resources: allowedPrefixes.map((prefix) =>
        ...
        : envName === 'prod'
          ...
          : envName === 'staging'
            ...
            : envName === 'dev'
              ? 1
              : undefined)
    const ingestFanoutTier2DesiredCount =
      toOptionalNumber(
      ...
      (envName === 'prod'
        ...
        : envName === 'staging'
          ...
          : envName === 'dev'
            ? 1
            : undefined)
    const notificationsDesiredCount =
      toOptionalNumber(
      ...
      (envName === 'prod'
        ...
        : envName === 'staging'
          ...
          : envName === 'dev'
            ? 1
            : undefined)
    const alertEvaluationServiceEnabled =
      toOptionalBool(
    ...
    const alertEvaluationDesiredCount =
      toOptionalNumber(
    ...
    if (
      planeBDbPoolMax < 1 ||
      planeBDbPoolMin < 0 ||
      planeBDbPoolMin > planeBDbPoolMax
    ) {
      throw new Error(
    ...
    const shouldRequireQuoteRefreshQueue =
      quoteRefreshQueueMode !== 'off' && b2cRefreshServiceEnabled
    const shouldRequireFxRateRefreshQueue =
      fxRateRefreshQueueMode !== 'off' && fxRateRefreshServiceEnabled
    const shouldRequireExportJobQueue =
      exportJobQueueMode !== 'off' && exportServiceEnabled
    ...
    const shouldRequireAgentFailureQueue =
      agentOrchestratorServiceEnabled && agentFailureQueueMode !== 'off'
    const shouldRequireAgentStressQueue =
      stressResponderServiceEnabled && agentStressQueueMode !== 'off'
    const shouldRequireToolRequestQueue =
      agentOrchestratorServiceEnabled && toolRequestQueueMode !== 'off'
    const shouldRequireNormalizationQueue =
      normalizationServiceEnabled && normalizationQueueMode !== 'off'
    ...
    if (shouldRequireQuoteRefreshQueue) {
      queueWorkerRequiredConfig.push([
        ...
        queues.quoteRefreshQueue?.queueUrl,
      ])
    }
    if (shouldRequireFxRateRefreshQueue) {
      queueWorkerRequiredConfig.push([
        'FX_RATE_REFRESH_QUEUE_URL',
        queues.fxRateRefreshQueue?.queueUrl,
      ])
    }
    if (shouldRequireExportJobQueue) {
      queueWorkerRequiredConfig.push([
        'EXPORT_JOB_QUEUE_URL',
        queues.exportJobQueue?.queueUrl,
      ])
      queueWorkerRequiredConfig.push([
        'EXPORTS_S3_BUCKET',
        storage.exportsBucket?.bucketName,
      ])
    }
    ...
    const requiresAgentLlmConfig =
      agentOrchestratorServiceEnabled || stressResponderServiceEnabled
    ...
    const enableWaf =
      toOptionalBool(
        this.node.tryGetContext('enableWaf') ?? process.env.ENABLE_WAF,
      ) ?? envName !== 'dev'
    ...
    if (
      (envName === 'staging' || envName === 'prod') &&
      planeAAdminEmails.length === 0
    ) {
      throw new Error(
        'PLANE_A_ADMIN_EMAILS is required in staging and production.',
      )
    }
    ...
    const planeAAdminRevocationFailClosed =
      cdkContext.planeAAdminRevocationFailClosed ??
    ...
    if (
      (envName === 'staging' || envName === 'prod') &&
      planeAAdminIpAllowlist.length === 0
    ) {
      throw new Error(
    ...
    const devPaused =
      envName === 'dev'
        ? (toOptionalBool(
            this.node.tryGetContext('devPaused') ?? process.env.DEV_PAUSED,
          ) ?? false)
        : false
    ...
    const devNightlyPauseTimezone =
      ...
      'America/New_York'
    const stagingBusinessHoursEnabled =
      envName === 'staging'
        ? (toOptionalBool(
    ...
    const resolvedOpsResumeAllowlist =
      opsResumeAllowlist.length > 0
        ? opsResumeAllowlist
        : opsPauseAllowlist.length > 0
          ...
          : envName === 'prod'
            ...
            : envName === 'dev'
              ? minimalMode
                ? ['alert-evaluation-worker', 'alert-evaluation-weekly']
                : []
              : []
    ...
    const resolvedPurgeQueueAllowlist =
      purgeQueueAllowlist.length > 0
        ? purgeQueueAllowlist
        : defaultPurgeQueueAllowlist
    const dedupedPurgeQueueAllowlist = [
      ...new Set(
        resolvedPurgeQueueAllowlist
    ...
    const resolvedPurgeQueueUrls = dedupedPurgeQueueAllowlist.map(
      (suffix) =>
        purgeQueueTargetsBySuffix[
          suffix as keyof typeof purgeQueueTargetsBySuffix
        ].url,
    )
    ...
    const planeADbHost =
      database.proxy?.endpoint ?? database.cluster.clusterEndpoint.hostname
    ...
    const planeADbTimeoutPolicy =
      planeADbConnectionRoute === 'proxy'
        ? 'proxy-guarded'
        : 'server-statement-timeout'
    const planeBDbTimeoutPolicy =
      planeBDbConnectionRoute === 'proxy'
        ? 'proxy-guarded'
        : 'server-statement-timeout'
    const planeCDbTimeoutPolicy =
      planeCDbConnectionRoute === 'proxy'
        ? 'proxy-guarded'
        : 'server-statement-timeout'
    ...
    const runtimeStack = new RuntimeNestedStack(this, 'Runtime', {
      ...
      taskOptions: {
        ...
        planeAAdminEmails:
          planeAAdminEmails.length > 0
            ? planeAAdminEmails.join(',')
            : undefined,
        planeAAdminIpAllowlist:
          planeAAdminIpAllowlist.length > 0
            ? planeAAdminIpAllowlist.join(',')
            : undefined,
        planeACorsOrigins:
          planeACorsOrigins.length > 0
            ? planeACorsOrigins.join(',')
            : undefined,
        planeACorsAllowedHeaders:
          planeACorsAllowedHeaders.length > 0
            ? planeACorsAllowedHeaders.join(',')
            : undefined,
        planeACorsAllowedMethods:
          planeACorsAllowedMethods.length > 0
            ? planeACorsAllowedMethods.join(',')
            : undefined,
        ...
        triangulationEnabled: toEnvBoolString(triangulationEnabled),
        stressDetectionEnabled: toEnvBoolString(stressDetectionEnabled),
        emitObservations: toEnvBoolString(emitObservations),
        moduleEmitObservationsDefault:
          toEnvBoolString(moduleEmitObservationsDefault),
      ...
      serviceOptions: {
        ...
        exportWorkerDesiredCount: exportServiceEnabled
          ...
          : 0,
        agentOrchestratorDesiredCount: agentOrchestratorServiceEnabled
          ? (agentOrchestratorDesiredCount ?? 0)
          : 0,
        stressResponderDesiredCount: stressResponderServiceEnabled
          ? (stressResponderDesiredCount ?? 0)
          : 0,
        normalizationWorkerDesiredCount: normalizationServiceEnabled
    ...
    const api = createApi(this, {
      ...
      planeCThrottleBurst,
      planeADomainName:
        this.node.tryGetContext('planeADomainName') ??
        process.env.PLANE_A_DOMAIN_NAME,
      planeACertificateArn:
        this.node.tryGetContext('planeACertificateArn') ??
        process.env.PLANE_A_CERT_ARN,
      planeAHostedZoneId:
        this.node.tryGetContext('planeAHostedZoneId') ??
        process.env.PLANE_A_HOSTED_ZONE_ID,
      planeAHostedZoneName:
        this.node.tryGetContext('planeAHostedZoneName') ??
        process.env.PLANE_A_HOSTED_ZONE_NAME,
    })
    ...
    const edgeStack = new EdgeNestedStack(this, 'Edge', {
      ...
      frontendOptions: {
        ...
        frontendCertificateArn:
          this.node.tryGetContext('frontendCertificateArn') ??
          process.env.FRONTEND_CERT_ARN,
        frontendHostedZoneId:
          this.node.tryGetContext('frontendHostedZoneId') ??
          process.env.FRONTEND_HOSTED_ZONE_ID,
        frontendHostedZoneName:
          this.node.tryGetContext('frontendHostedZoneName') ??
          process.env.FRONTEND_HOSTED_ZONE_NAME,
        enableFrontend,
      },
    })
    ...
    const ingestFanoutTier1Baseline =
      ingestFanoutMode === 'queue'
        ? (ingestFanoutTier1DesiredCount ?? queueWorkerBaseline)
        : 0
    const ingestFanoutTier2Baseline =
      ingestFanoutMode === 'queue'
        ? (ingestFanoutTier2DesiredCount ??
          Math.max(0, ingestFanoutTier1Baseline - 1))
        : 0
    const goldLiveBaseline =
      goldLiveQueueMode === 'queue'
        ? (goldLiveDesiredCount ?? queueWorkerBaseline)
        : 0
    const notificationsBaseline =
      notificationsMode === 'queue'
        ? (notificationsDesiredCount ?? queueWorkerBaseline)
        : 0
    const opsAlertsBaseline =
      opsAlertsMode === 'queue'
        ? (opsAlertsDesiredCount ?? queueWorkerBaseline)
        : 0
    const b2cRefreshBaseline = b2cRefreshServiceEnabled
      ? (b2cRefreshDesiredCount ?? 0)
      : 0
    const fxRateRefreshBaseline = fxRateRefreshServiceEnabled
      ? (fxRateRefreshDesiredCount ?? 0)
      : 0
    const alertEvaluationBaseline = alertEvaluationServiceEnabled
      ? (alertEvaluationDesiredCount ?? 0)
      : 0
    const exportWorkerBaseline = exportServiceEnabled
      ? (exportWorkerDesiredCount ?? 0)
      : 0
    ...
    addManagedService(
      ecsServices.ingestFanoutTier1Service,
      ingestFanoutTier1Baseline,
    )
    addManagedService(
      ecsServices.ingestFanoutTier2Service,
      ingestFanoutTier2Baseline,
    )
    addManagedService(ecsServices.goldLiveService, goldLiveBaseline)
    addManagedService(
      ecsServices.notificationsQueueService,
      notificationsBaseline,
    )
    addManagedService(ecsServices.opsAlertsQueueService, opsAlertsBaseline)
    addManagedService(
      ecsServices.alertEvaluationService,
      alertEvaluationBaseline,
    )
    addManagedService(ecsServices.exportWorkerService, exportWorkerBaseline)
    addManagedService(
      ecsServices.agentOrchestratorService,
      agentOrchestratorBaseline,
    )
    addManagedService(
      ecsServices.stressResponderService,
      stressResponderBaseline,
    )
    addManagedService(
      ecsServices.normalizationWorkerService,
      normalizationWorkerBaseline,
    )
    ...
    const opsStack = new OpsNestedStack(this, 'Ops', {
      ...
      pipelineOptions: {
        ...
        publicSiteUrl: frontendBaseUrl,
        publicApiBase:
          (this.node.tryGetContext('planeADomainName') ?? process.env.PLANE_A_DOMAIN_NAME)
            ? `https://${this.node.tryGetContext('planeADomainName') ?? process.env.PLANE_A_DOMAIN_NAME}/api/v1`
            : undefined,
        publicSupabaseUrl,
      ...
      opsPauseOptions: {
        ...
        redisSubnetGroupName:
          cache.subnetGroup.cacheSubnetGroupName ?? cache.subnetGroup.ref,
        ...
        redisReplicasPerNodeGroup:
          envName === 'prod' ? (resolvedProdRedisReplicasPerNodeGroup ?? 1) : 0,
    ...
    if (monitoring) {
      ...
      for (const metricInfo of exportsRequestMetrics) {
        ...
        const requestSpikeAlarm = new Alarm(
          this,
          `ExportsRequestSpike${metricInfo.label}`,
          {
        ...
        requestSpikeAlarm.addAlarmAction(
          new SnsAction(snsSubscriptions.opsTopic),
        )
      }

      if (snowflakePartnerRole) {
        const assumeRoleFailureRule = new Rule(
          this,
          'SnowflakeAssumeRoleFailureRule',
          {
            ruleName: `remit-scout-${envName}-snowflake-assume-role-failure`,
            description:
              'Alerts on failed STS AssumeRole for Snowflake partner role.',
            eventPattern: {
              source: ['aws.sts'],
    ...
    if (envName === 'staging' || envName === 'prod') {
      ...
      const failedTaskStartRule = new Rule(
        this,
        'ScheduledTaskFailedToStartRule',
        {
          ruleName: `remit-scout-${envName}-scheduled-task-failed-to-start`,
          description:
            'Detects ECS scheduled task startup failures (TaskFailedToStart).',
          eventPattern: {
            source: ['aws.ecs'],
      ...
      const failedTaskStartAlarm = new Alarm(
        this,
        'ScheduledTaskFailedToStartAlarm',
        {
          ...
          treatMissingData: TreatMissingData.NOT_BREACHING,
          alarmDescription:
            'Scheduled ECS task startup failure detected in the last 5 minutes.',
        },
      )
      failedTaskStartAlarm.addAlarmAction(
        new SnsAction(snsSubscriptions.opsTopic),
      )
    }
    ...
    if (envName === 'dev' && devNightlyPauseEnabled) {
      ...
      const schedulerInvokeRole = new Role(
        this,
        'DevNightlyPauseSchedulerRole',
        {
      ...
      if (costGuardrailTopic) {
        costGuardrailTopic.addSubscription(
          new LambdaSubscription(opsPause.controllerFunction),
        )
      }
    }

    if (envName === 'staging' && stagingBusinessHoursEnabled) {
      const businessHoursDlq = new Queue(
        this,
        'StagingBusinessHoursSchedulerDlq',
        {
      ...
      const schedulerInvokeRole = new Role(
        this,
        'StagingBusinessHoursSchedulerRole',
        {
          assumedBy: new ServicePrincipal('scheduler.amazonaws.com'),
        },
      )
      ...
      const dlqAlarm = new Alarm(
        this,
        'StagingBusinessHoursSchedulerDlqAlarm',
        {
