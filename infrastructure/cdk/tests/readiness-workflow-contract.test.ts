...
test('staging readiness records restored service evidence from the parsed service list used for ECS resume', () => {
  assert.match(
    readinessWorkflow,
    /Resume staging operational services for parity evidence[\s\S]*services_json="\$\(jq -c 'fromjson\? \/\/ \.' <<<"\$\{services_json\}"\)"[\s\S]*baseline_json="\$\(jq -c 'fromjson\? \/\/ \.' <<<"\$\{baseline_json\}"\)"[\s\S]*jq -cn --argjson services "\$\{services_json\}" --argjson baseline "\$\{baseline_json\}"[\s\S]*mapfile -t service_names < <\(jq -r '\.\[\]\? \| strings \| select\(length > 0\)' "\$\{resolved_services_path\}"\)[\s\S]*--slurpfile services "\$\{resolved_services_path\}"/,
  )
})
...
test('staging readiness applies candidate migrations inside staging ECS runtime', () => {
  assert.match(
    readinessWorkflow,
    /Resolve staging DbMigrate ECS runtime[\s\S]*Resolve current staging migration version via ECS[\s\S]*READINESS_CURRENT_MIGRATION_VERSION[\s\S]*Build pending exact-SHA migration payload \(staging readiness\)[\s\S]*READINESS_PENDING_MIGRATIONS_B64[\s\S]*Apply repo migrations from exact SHA via ECS \(staging readiness\)[\s\S]*aws ecs run-task[\s\S]*Persist ECS migration sync evidence \(staging readiness\)[\s\S]*STAGING_MIGRATION_SYNC_RESULT_PATH/,
  )
})
